...
class ITransformerHeadCustom(nn.Module):
    ...
    def __init__(
        ...
        self.instance_norm_time = (
            instance_norm_time  # (optional) per-variable norm over time
        )
        ...
        self.pos_enc_vars = PositionalEncoding(
            d_model=d_model, max_len=max_vars + (1 if self.use_cls else 0)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x, _, _ = self._maybe_instance_norm_time(
            x
        )  # we can expose mu/sigma if you want reversible norm
        ...
        if self.output_mode == "pooled":
            ...
            H_exp = H.unsqueeze(2).expand(
                B, self.pred_len, C, self.d_model
            )  # [B, T_pred, C, D]
            Tok_exp = tok_no_cls.unsqueeze(1).expand(
                B, self.pred_len, C, self.d_model
            )  # [B, T_pred, C, D]
            fused = H_exp + Tok_exp
