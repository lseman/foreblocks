...
class TemporalFusionTransformerHead(nn.Module):
    ...
    def __init__(
        ...
        quantiles: tuple[float, ...] | None = None,  # e.g., (0.1, 0.5, 0.9)
    ):
        super().__init__()
    ...
    def forward(
        ...
        time_hist: torch.Tensor | None = None,
        time_fut: torch.Tensor | None = None,
        ...
        z_enc, w_enc = self._build_encoder_input(
            x_hist_vars
        )  # [B, L_in, d_model], [B, L_in, V_enc]

        # 2) Static enrichment (context)
        c = self._static_context(x_static)  # [B, 1, d_model] or None
        if c is not None:
            z_enc = self.enrich_enc(
                z_enc, context=c.expand(-1, z_enc.size(1), -1)
            )  # GRN(x || c)
        ...
        z_dec_in, w_dec = self._build_decoder_input(
            x_fut_vars, prev_tgt
        )  # [B, L_out, d_model], [B, L_out, V_dec]
        if c is not None:
            z_dec_in = self.enrich_dec(
                z_dec_in, context=c.expand(-1, z_dec_in.size(1), -1)
            )
