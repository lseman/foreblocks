...
class MLTrackerClient:
    ...
    def log_metric(
        self, key: str, value: float, step: int = 0, run_id: str | None = None
    ):
        """Log a metric"""
        rid = run_id or self._active_run_id
        if not rid:
    ...
    def log_metrics(
        self, metrics: dict[str, float], step: int = 0, run_id: str | None = None
    ):
    ...
    # Artifact methods
    def log_artifact(
        self, local_path: str, artifact_path: str = "", run_id: str | None = None
    ):
        """Upload an artifact"""
        rid = run_id or self._active_run_id
        if not rid:
        ...
        with open(local_file, "rb") as f:
            ...
            response = requests.post(
                f"{self.base_url}/api/runs/{rid}/artifacts", files=files, params=params
            )
...
if __name__ == "__main__":
    ...
    with start_run(client, "sklearn_experiments", "random_forest_v1"):
        ...
        for epoch in range(5):
            client.log_metrics(
                {"train_acc": 0.7 + epoch * 0.05, "val_acc": 0.65 + epoch * 0.045},
                step=epoch,
            )
