...
export function App() {
	...
	useEffect(() => {
		...
		const parsed = parseUrlState();
		const urlState =
			parsed?.experimentName === activeExperiment ? parsed.state : {};
		...
		setSelectedRunIds(
			Array.isArray(source.selectedRunIds)
				? source.selectedRunIds.slice(0, 80)
				: [],
		);
		setCompareData(null);
		setCompareHistory({});
		...
		void (async () => {
			try {
				const [data, overviewData] = await Promise.all([
					runQuery.trim()
						? searchRuns(activeExperiment, runQuery.trim())
						: getRuns(activeExperiment),
					getOverview(activeExperiment),
				]);
				setRunsRaw(data);
	...
	const statusOptions = useMemo(() => {
		...
		const extra = [...statusCounts.keys()]
			.filter((x) => !STATUS_ORDER.includes(x))
			.sort();
	...
	const metricKeys = useMemo(() => {
		const fromOverview =
			overview?.metric_catalog?.map((entry) => entry.key) ?? [];
		const merged = new Set([
			...fromOverview,
			...extractNumericMetricKeys(sortedRuns),
		]);
		return [...merged].sort();
	}, [overview, sortedRuns]);
	const paramKeys = useMemo(
		() => extractNumericParamKeys(sortedRuns),
		[sortedRuns],
	);
	...
	const sweepAxisOptions = useMemo(
		() => [
			...metricKeys.map((k) => ({
				value: `metric:${k}` as SweepAxisMode,
				label: `metric: ${k}`,
			})),
			...paramKeys.map((k) => ({
				value: `param:${k}` as SweepAxisMode,
				label: `param: ${k}`,
			})),
		],
	...
	const filteredRuns = useMemo(() => {
		...
		const useMetric =
			Boolean(metricFilterKey) &&
			(Number.isFinite(min) || Number.isFinite(max));

		return sortedRuns.filter((run) => {
			if (activeStatus !== "ALL" && (run.status ?? "UNKNOWN") !== activeStatus)
				return false;
	...
	const groupedRuns = useMemo(
		() => groupRuns(filteredRuns, groupMode),
	...
	const finishedRuns = useMemo(
		() =>
			filteredRuns.filter((r) => (r.status ?? "").toUpperCase() === "FINISHED")
				.length,
		[filteredRuns],
	);
	...
	const runningRuns = useMemo(
		() =>
			filteredRuns.filter((r) => (r.status ?? "").toUpperCase() === "RUNNING")
				.length,
		[filteredRuns],
	);
	...
	const failedRuns = useMemo(
		() =>
			filteredRuns.filter((r) => (r.status ?? "").toUpperCase() === "FAILED")
				.length,
		[filteredRuns],
	);
	...
	const primaryMetric = useMemo(
		() => overview?.primary_metric ?? pickPrimaryMetricKey(filteredRuns),
	...
	const latestRun = useMemo(
		() =>
			[...filteredRuns].sort(
				(a, b) => parseTime(b.start_time) - parseTime(a.start_time),
			)[0] ?? null,
		[filteredRuns],
	);
	...
	const metricCatalogTop = useMemo(
		() => (overview?.metric_catalog ?? []).slice(0, 5),
		[overview],
	);

	const bestRunSummary = overview?.best_run ?? null;
	const overviewTotals = overview?.totals ?? null;
	...
	const leaderboardRows = useMemo(() => {
		...
		return filteredRuns
			...
			.filter(
				(entry): entry is { run: Run; value: number } => entry.value !== null,
			)
			.sort((a, b) =>
				objective === "min" ? a.value - b.value : b.value - a.value,
			)
			.slice(0, 8);
	}, [filteredRuns, leaderboardMetric, leaderboardObjective]);

	const parallelAvailableDims = useMemo(
		() => collectParallelDimensions(filteredRuns),
		[filteredRuns],
	);
	...
	useEffect(() => {
		...
		const safe = parallelDims.filter((dim) =>
			parallelAvailableDims.includes(dim),
		);
		if (!safe.length) {
			setParallelDims(
				defaultParallelDimensions(parallelAvailableDims, primaryMetric),
			);
			...
			try {
				...
				setError(
					err instanceof Error
						? err.message
						: "Failed to load compare metric history",
				);
				setCompareHistory({});
			} finally {
	...
	const sweepPoints = useMemo(() => {
		if (!sweepXAxis || !sweepYAxis)
			return [] as Array<{ run: Run; x: number; y: number }>;
	...
	const sweepTopRuns = useMemo(() => {
		if (!sweepPoints.length)
			return [] as Array<{ run: Run; x: number; y: number }>;
		const objective =
			sweepYAxis.startsWith("metric:") &&
			metricLowerIsBetter(sweepYAxis.slice("metric:".length))
				? "min"
				: "max";
		return [...sweepPoints]
	...
	function moveModule(
		sourceId: DashboardModuleId,
		targetId: DashboardModuleId,
	): void {
	...
	useEffect(() => {
		...
		if (runDetailMetricKey && runDetailMetricKeys.includes(runDetailMetricKey))
			return;

		const preferred =
			(primaryMetric && runDetailMetricKeys.includes(primaryMetric)
				? primaryMetric
	...
	const parallelActiveDims = useMemo(
		() =>
			parallelDims
				.filter((dim) => parallelAvailableDims.includes(dim))
				.slice(0, 6),
	...
	const paletteActions = useMemo<PaletteAction[]>(() => {
			...
			actions.push({
				...
				run: () => {
					const name = window.prompt(
						"Saved view name:",
						`view-${new Date().toISOString().slice(0, 16)}`,
					);
			...
			for (const entry of savedViews) {
				actions.push({
					...
					run: () => {
						...
						setSelectedRunIds(
							Array.isArray(s.selectedRunIds)
								? s.selectedRunIds.slice(0, 80)
								: [],
						);
					},
				});
		...
		return actions.filter((x) =>
			`${x.title} ${x.meta ?? ""}`.toLowerCase().includes(q),
		);
	}, [
		paletteQuery,
		activeExperiment,
	...
	const layoutById = new Map(
		moduleLayout.map((item) => [item.id, item] as const),
	);
	...
	const dashboardModules: DashboardModuleDefinition[] = [
		...
		},
		{
			...
			body: compareSection?.body ?? (
				<div className="module-empty">
					Choose at least two runs to compare them here.
				</div>
			),
		},
	];
	...
	const orderedDashboardModules = (() => {
		const defsById = new Map(
			dashboardModules.map((module) => [module.id, module] as const),
		);
	...
	function renderDashboardModule(
		...
		const span =
			layoutById.get(module.id)?.span ??
			DEFAULT_MODULE_LAYOUT.find((item) => item.id === module.id)?.span ??
			1;
						...
						<div className="module-layout-tools">
							{[1, 2, 3].map((option) => (
								<button
									...
									onClick={() =>
										updateModuleSpan(module.id, option as DashboardModuleSpan)
									}
			...
			<main className="main-content">
				<header className="top-bar">
					...
					<div className="actions">
						<span
							className={`connection-pill ${health === "healthy" ? "online" : health === "offline" ? "offline" : "pending"}`}
						>
							API {health}
						</span>
						<button
									...
									return (
										...
										>
											<input
												...
												onChange={(e) =>
													setRunSelected(run.run_id, e.target.checked)
												}
											/>
											<div className="run-rail-main">
												<div className="run-rail-head">
													<span className="run-rail-name">
														{run.name ?? run.run_id}
													</span>
													<span
														className={`run-rail-status rail-status-${runStatus}`}
													>
														{run.status ?? "UNKNOWN"}
													</span>
												</div>
					...
					<section className="view-container">
						...
						{!activeExperiment ? (
							<div className="empty-state">
								...
								<p>
									Open the project picker to explore runs, metrics, charts, and
									artifacts.
								</p>
								<button
									type="button"
							...
							<>
								<div className="view-header">
									<div>
										...
										<p className="view-subtitle">
											{loadingRuns
												? "Loading runs..."
												: `${filteredRuns.length} / ${sortedRuns.length} runs visible`}
										</p>
									</div>
								...
								<div className="summary-grid">
									<KpiCard
										label="Total Runs"
										value={overviewTotals?.runs ?? filteredRuns.length}
										tone="gold"
									/>
									<KpiCard
										label="Running"
										value={overviewTotals?.running ?? runningRuns}
										tone="cyan"
									/>
									<KpiCard
										label={failedRuns ? "Failed" : "Finished"}
										value={
											overviewTotals?.failed ||
											overviewTotals?.finished ||
											failedRuns ||
											finishedRuns
										}
										tone="mint"
									...
									<KpiCard
										...
										value={formatDuration(
											overviewTotals?.avg_duration ?? avgDuration,
										)}
										tone="gold"
									/>
								</div>
								...
								<section className="project-intelligence">
									<div className="project-intelligence-main">
										<div className="exp-kicker">Project Intelligence</div>
										<h3>
											{bestRunSummary
												? `${bestRunSummary.metric_key}: ${formatMetricValue(bestRunSummary.metric_value)}`
												: primaryMetric
													? `${primaryMetric}: waiting for comparable runs`
													: "No primary metric yet"}
										</h3>
										<p>
											{bestRunSummary
												? `Best ${bestRunSummary.objective === "min" ? "minimum" : "maximum"} run is ${bestRunSummary.name ?? bestRunSummary.run_id} at step ${bestRunSummary.metric_step}.`
												: "Log numeric metrics to unlock best-run, sweep, and importance summaries."}
										</p>
									</div>
									<div className="project-intelligence-side">
										<div className="intelligence-stat">
											<span>Success Rate</span>
											<strong>
												{(overviewTotals?.success_rate ?? successRate).toFixed(
													1,
												)}
												%
											</strong>
										...
										<div className="intelligence-stat">
											...
											<strong>
												{overview?.metric_catalog.length ?? metricKeys.length}
											</strong>
										</div>
										<div className="intelligence-stat">
											<span>Param Keys</span>
											<strong>
												{overview?.param_catalog.length ?? paramKeys.length}
											</strong>
										</div>
									</div>
									<div className="metric-catalog-strip">
										{metricCatalogTop.map((entry) => (
											<button
												key={`metric-catalog-${entry.key}`}
												type="button"
												className={`metric-catalog-pill ${leaderboardMetric === entry.key ? "active" : ""}`}
												onClick={() => {
													setLeaderboardMetric(entry.key);
													setImportanceTarget(entry.key);
													setSweepYAxis(`metric:${entry.key}`);
												}}
											>
												<span>{entry.key}</span>
												<b>{entry.run_count} runs</b>
											</button>
										))}
										{!metricCatalogTop.length && (
											<div className="metric-catalog-empty">
												Metric catalog will appear after runs log metrics.
											</div>
										)}
									</div>
								</section>
								...
								<div className="insights-strip">
									<article className="insight-card">
										...
										<div className="insight-note">
											{leaderboardRows[0]
												? `Best run: ${leaderboardRows[0].run.name ?? leaderboardRows[0].run.run_id}`
												: "Log metrics to unlock ranking"}
										</div>
									</article>
									<article className="insight-card">
										...
										<div className="insight-note">
											{finishedRuns}/{filteredRuns.length} runs finished
										</div>
									</article>
									<article className="insight-card">
										<div className="insight-label">Recent Activity</div>
										<div className="insight-value">
											{overviewTotals?.recent_24h ?? last24hRuns} runs / 24h
										</div>
										<div className="insight-note">
											{latestRun
												? `Latest: ${latestRun.name ?? latestRun.run_id} · ${formatCompactDate(latestRun.start_time)}`
								...
								<div className="dashboard-board">
									{orderedDashboardModules.map((module) =>
										renderDashboardModule(module),
									)}
								</div>
								...
								<div className="advanced-controls">
									...
									<select
										className="sort-select"
										value={metricFilterKey}
										onChange={(e) => setMetricFilterKey(e.target.value)}
									>
									...
									<table className="runs-table">
										...
										<tbody>
											{groupedRuns.map((group, groupIdx) => (
												<Fragment
													key={`grp-${group.label ?? "none"}-${groupIdx}`}
												>
													{group.label && (
													...
													{group.runs.slice(0, 250).map((run) => (
														<tr
															key={run.run_id}
															className={`run-row ${selectedSet.has(run.run_id) ? "is-selected" : ""}`}
														>
															...
															<td>
																<span
																	className={`status-badge status-${(run.status ?? "UNKNOWN").toUpperCase()}`}
																>
																	{run.status ?? "UNKNOWN"}
																</span>
															</td>
															...
															<td>
																<div className="metric-preview">
																	...
																	{!Object.keys(run.metrics ?? {}).length && (
																		<span className="metric-chip">-</span>
																	)}
																</div>
															</td>
															<td>
