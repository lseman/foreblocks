...
class StreamlinedDARTSAnalyzer:
    ...
    def _create_analysis_dataframe(self) -> pd.DataFrame:
        ...
        for candidate in self.candidates:
            ...
            print(f"Extracted features for candidate {candidate['candidate_id']}: {row}")
            data.append(row)
    ...
    def _add_zcm_features(self, candidate: Dict, row: Dict):
        ...
        print(f"Processing candidate {candidate['candidate_id']} with metrics: {metrics}")
        ...
        print(f"Added zero-cost metrics for candidate {candidate['candidate_id']}: {row}")
    ...
    def _add_operation_features(self, candidate: Dict, row: Dict):
        ...
        all_ops = ['Identity', 'TimeConv', 'GRN', 'Wavelet', 'Fourier', 
                   'TCN', 'ResidualMLP', 'ConvMixer', 'MultiScaleConv', 'PyramidConv']
    ...
    def _extract_component_info(self, model, component_attr: str, component_type: str, row: Dict):
        ...
        # Get component types
        components_attr = f"{component_type}s" if not component_type.endswith('r') else f"{component_type}s"
        components = getattr(component, components_attr, [])

        #print(pato)
        if components:
            print(f"Extracting {component_type} information for candidate {row['candidate_id']}")
            ...
            for i, comp_type in enumerate(component_types):
                clean_name = comp_type.replace('Encoder', '').replace('Decoder', '').replace('RNN', '').lower()
    ...
    def _add_training_status(self, candidate: Dict, row: Dict):
        ...
        row['is_top_candidate'] = any(tc['candidate_id'] == candidate['candidate_id'] 
    ...
    def _add_final_architecture_info(self, trained_info: Dict, row: Dict):
        ...
        try:
            ...
            model = search_results.get('model') if search_results else trained_info.get('model')
            
            if model is None:
                return
            
            # Final encoder choice
            self._extract_final_component_choice(model, 'forecast_encoder', 'encoder', row)
            
            # Final decoder choice
            self._extract_final_component_choice(model, 'forecast_decoder', 'decoder', row)
    ...
    def _extract_final_component_choice(self, model, component_attr: str, component_type: str, row: Dict):
        ...
        if selected_idx < len(components):
            ...
            clean_name = selected_component.replace('Encoder', '').replace('Decoder', '').replace('RNN', '')
    ...
    def _analyze_all_patterns(self) -> Dict[str, Any]:
        ...
        for comp_type in component_types:
            patterns['component_analysis'][comp_type] = self._analyze_component_patterns(comp_type)
    ...
    def _analyze_component_patterns(self, component_type: str) -> Dict[str, Any]:
        ...
        for comp_value in component_values:
            ...
            freq_data[comp_value] = {
                ...
                'avg_weight': self.analysis_df[mask][weight_col].mean() if weight_col in self.analysis_df.columns and mask.sum() > 0 else 0
            }
    ...
    def _analyze_attention_patterns(self) -> Dict[str, Any]:
        ...
        attention_stats = {
            ...
            'top_candidates_usage': self.analysis_df[self.analysis_df['is_top_candidate']]['uses_attention'].sum(),
            'trained_usage': self.analysis_df[self.analysis_df['was_trained']]['uses_attention'].sum()
        }
        
        if 'likely_attention' in self.analysis_df.columns:
            attention_choices = self.analysis_df['likely_attention'].value_counts().to_dict()
    ...
    def _analyze_final_patterns(self) -> Dict[str, Any]:
        ...
        if 'final_uses_attention' in trained_df.columns:
            final_analysis['final_attention_usage'] = trained_df['final_uses_attention'].sum()
            final_analysis['final_attention_rate'] = trained_df['final_uses_attention'].mean()
            
            if 'final_attention_choice' in trained_df.columns:
                final_analysis['final_attention_choices'] = trained_df['final_attention_choice'].value_counts().to_dict()
    ...
    def analyze_correlations(self) -> pd.DataFrame:
        ...
        arch_features = (['num_operations', 'hidden_dim', 'num_cells', 'num_nodes'] +
                        [col for col in numeric_cols if any(prefix in col for prefix in 
                         ['op_', 'encoder', 'decoder', 'attention'])])
        ...
        for arch_feat in arch_features:
            for perf_metric in performance_metrics:
                if arch_feat in self.analysis_df.columns and perf_metric in self.analysis_df.columns:
                    clean_data = self.analysis_df[[arch_feat, perf_metric]].dropna()
                    if len(clean_data) > 1 and clean_data[arch_feat].var() > 0:
                        corr, p_value = stats.pearsonr(clean_data[arch_feat], clean_data[perf_metric])
                        corr_data.append({
    ...
    def plot_encoder_decoder_analysis(self, figsize: Tuple[int, int] = (12, 6)) -> plt.Figure:
        """Create focused encoder/decoder/attention analysis plot"""
        fig, axes = plt.subplots(1, 3, figsize=figsize)
        fig.suptitle('Encoder/Decoder/Attention Architecture Analysis', fontsize=16, fontweight='bold')
    ...
    def plot_operation_analysis(self, figsize: Tuple[int, int] = (16, 10)) -> plt.Figure:
        ...
        return fig
    def plot_correlation_analysis(self, figsize: Tuple[int, int] = (14, 8)) -> plt.Figure:
        """Create focused correlation analysis plot"""
        fig, axes = plt.subplots(1, 2, figsize=figsize)
        fig.suptitle('Architecture-Performance Correlation Analysis', fontsize=16, fontweight='bold')
    ...
    def plot_performance_analysis(self, figsize: Tuple[int, int] = (16, 8)) -> plt.Figure:
    ...
    def _plot_encoder_analysis(self, ax):
        ...
        bars1 = ax.bar(x - width, overall_counts, width, label='All Candidates', 
                      alpha=0.8, color='skyblue')
        bars2 = ax.bar(x, top_counts, width, label='Top Candidates', 
                      alpha=0.8, color='orange')
        bars3 = ax.bar(x + width, trained_counts, width, label='Trained', 
        ...
        ax.set_ylabel('Count')
        ax.set_xticks(x)
        ax.set_xticklabels([enc.replace('Encoder', '') for enc in encoders], rotation=45, ha='right')
        ax.legend()
    ...
    def _plot_decoder_analysis(self, ax):
        ...
        if not decoder_data:
            ax.text(0.5, 0.5, 'No Decoder Data', ha='center', va='center', transform=ax.transAxes)
        ...
        ax.set_ylabel('Count')
        ax.set_xticks(x)
        ax.set_xticklabels([dec.replace('Decoder', '') for dec in decoders], rotation=45, ha='right')
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def _plot_attention_analysis(self, ax):
        ...
        if not attention_data or 'attention_choices' not in attention_data:
            ...
            if attention_data:
                ...
                wedges, texts, autotexts = ax.pie(usage_counts, labels=categories, autopct='%1.1f%%',
    ...
    def _plot_final_encoder_choices(self, ax):
        ...
        if 'final_encoders' not in final_data:
            ax.text(0.5, 0.5, 'No Final Encoder Data', ha='center', va='center', transform=ax.transAxes)
        ...
        bars = ax.bar(range(len(encoder_names)), encoder_counts, alpha=0.8, color='lightblue')
        ...
        ax.set_xticklabels([name.replace('Encoder', '') for name in encoder_names], 
    ...
    def _plot_final_decoder_choices(self, ax):
        ...
        if 'final_rnn_types' in final_data:
            ...
            ax.text(0.5, 0.5, 'No Final Decoder Data', ha='center', va='center', transform=ax.transAxes)
        ...
        bars = ax.bar(range(len(choice_names)), choice_counts, alpha=0.8, color='lightcoral')
        ...
        ax.set_xticklabels([name.replace('Decoder', '') for name in choice_names], 
                          rotation=45, ha='right')
        ax.grid(True, alpha=0.3)
    ...
    def _plot_final_attention_choices(self, ax):
        ...
        if 'final_attention_choices' not in final_data:
            ...
            if 'final_attention_rate' in final_data:
                ...
                ax.text(0.5, 0.5, 'No Final Attention Data', ha='center', va='center', transform=ax.transAxes)
        ...
        wedges, texts, autotexts = ax.pie(choice_counts, labels=choice_names, autopct='%1.1f%%',
                                        colors=colors[:len(choice_names)], startangle=90)
    ...
    def _plot_operation_frequencies(self, ax):
        ...
        bars1 = ax.bar(x - width/2, overall_freq, width, label='All Candidates', alpha=0.7, color='lightblue')
        bars2 = ax.bar(x + width/2, trained_freq, width, label='Trained Models', alpha=0.7, color='orange')
    ...
    def _plot_architecture_complexity(self, ax):
        ...
        bars = ax.bar(complexity_bins.index, complexity_bins.values, alpha=0.7, color='lightgreen')
    ...
    def _plot_operation_success_rates(self, ax):
        ...
        bars = ax.bar(range(len(operations)), success_rates, alpha=0.7, color='mediumpurple')
    ...
    def _plot_top_correlations(self, ax):
        ...
        if top_corr.empty:
            ax.text(0.5, 0.5, 'No Strong Correlations', ha='center', va='center', transform=ax.transAxes)
            ax.set_title('Top Correlations')
            return
        
        # Create labels and colors
        labels = [f"{row['architectural_feature'][:15]}..." if len(row['architectural_feature']) > 15 
                 else row['architectural_feature'] for _, row in top_corr.iterrows()]
        ...
        for i, (bar, corr) in enumerate(zip(bars, correlations)):
            ax.text(bar.get_width() + 0.01 if corr > 0 else bar.get_width() - 0.01, 
    ...
    def _plot_performance_distributions(self, ax):
        ...
        top_scores = self.analysis_df[self.analysis_df['is_top_candidate']]['zero_cost_score'].dropna()
        trained_scores = self.analysis_df[self.analysis_df['was_trained']]['zero_cost_score'].dropna()
        
        ax.hist(all_scores, bins=20, alpha=0.5, label='All Candidates', color='lightblue')
        if not top_scores.empty:
            ax.hist(top_scores, bins=15, alpha=0.7, label='Top Candidates', color='orange')
        if not trained_scores.empty:
            ax.hist(trained_scores, bins=10, alpha=0.8, label='Trained Models', color='green')
    ...
    def _plot_training_success_analysis(self, ax):
        ...
        # Create quartiles
        quartiles = pd.qcut(self.analysis_df['zero_cost_score'], q=4, labels=['Q1', 'Q2', 'Q3', 'Q4'])
        ...
        bars = ax.bar(range(len(quartile_labels)), success_rates, alpha=0.7, color='lightcoral')
    ...
    def _plot_correlation_heatmap(self, ax):
        ...
        heatmap_data = corr_df.pivot(index='architectural_feature', 
    ...
    def generate_insights_report(self) -> str:
            ...
            for comp_type, data in component_data.items():
                if data:
                    ...
                    for comp_name, freq_data in data.items():
                        clean_name = comp_name.replace('Encoder', '').replace('Decoder', '')
                        avg_weight = freq_data.get('avg_weight', 0)
                        report.append(f"    {clean_name}: {freq_data['trained']} trained "
                                     f"(α={avg_weight:.3f})")
        ...
        if not strong_corr.empty:
            ...
            for _, row in strong_corr.head(5).iterrows():
                ...
                report.append(f"  {row['architectural_feature']} ↔ {row['performance_metric']}: "
        ...
        if component_data:
            for comp_type, data in component_data.items():
                if data:
                    best_comp = max(data.items(), key=lambda x: x[1]['trained'] / max(x[1]['overall'], 1))
                    success_rate = best_comp[1]['trained'] / max(best_comp[1]['overall'], 1)
                    clean_name = best_comp[0].replace('Encoder', '').replace('Decoder', '')
                    report.append(f"  • Best {comp_type}: {clean_name} "
        ...
        if attention_data:
            ...
            if usage_rate > 0.5:
                report.append(f"  • High attention usage ({usage_rate:.1%}) suggests complex dependencies")
            else:
                report.append(f"  • Low attention usage ({usage_rate:.1%}) indicates simpler patterns work well")
    ...
    def plot_architecture_space_exploration(self, figsize: Tuple[int, int] = (14, 6)) -> plt.Figure:
        ...
        scatter = axes[0].scatter(scatter_data['hidden_dim'], 
        ...
        axes[0].set_title('Architecture Space Exploration\n(Size=Cells, Color=Operations)')
        ...
        if len(self.analysis_df[self.analysis_df['is_top_candidate']]) > 0:
            ...
            axes[0].scatter(top_data['hidden_dim'], 
    ...
    def plot_metric_architecture_correlations(self, method: str = "pearson", figsize=(16, 10)) -> plt.Figure:
        """
        ...
        arch_cols = [col for col in df.columns if col in ['num_operations', 'hidden_dim', 'num_cells', 'num_nodes']
        ...
        sns.heatmap(corr_matrix, cmap="coolwarm", center=0, annot=True, fmt=".2f",
                    linewidths=0.5, cbar_kws={"label": f"{method.title()} Correlation"}, ax=ax)
        ax.set_title(f"{method.title()} Correlation: Zero-Cost Metrics vs Architectural Features", fontsize=14)
        plt.xticks(rotation=45, ha='right')
        plt.yticks(rotation=0)
        plt.tight_layout()
        return fig
    def plot_architecture_configurations(self, top_k: int = 20, figsize: Tuple[int, int] = (10, 6)) -> plt.Figure:
        """
        ...
        """
        if not all(col in self.analysis_df.columns for col in ['num_cells', 'num_nodes', 'hidden_dim']):
            fig, ax = plt.subplots(figsize=figsize)
            ax.text(0.5, 0.5, 'Missing columns in data', ha='center', va='center', transform=ax.transAxes)
        ...
        self.analysis_df['arch_config'] = self.analysis_df.apply(
            lambda row: f"{row['num_cells']}C-{row['num_nodes']}N-{row['hidden_dim']}H", axis=1
        )

        # Count frequencies
        config_counts = self.analysis_df['arch_config'].value_counts().sort_values(ascending=True).tail(top_k)

        # Create figure
        fig, ax = plt.subplots(figsize=figsize)
        sns.barplot(x=config_counts.values, y=config_counts.index, palette="Purples_d", ax=ax)
...
def analyze_darts_search(search_results: Dict[str, Any], 
                        save_plots: bool = True,
                        plot_dir: str = "./plots/") -> StreamlinedDARTSAnalyzer:
    """
    ...
    if save_plots:
        # Save all plots
        fig1.savefig(f"{plot_dir}/encoder_decoder_analysis.png", dpi=300, bbox_inches='tight')
        ...
        fig4.savefig(f"{plot_dir}/performance_analysis.png", dpi=300, bbox_inches='tight')
        fig5.savefig(f"{plot_dir}/architecture_space_exploration.png", dpi=300, bbox_inches='tight')
        fig6.savefig(f"{plot_dir}/metric_architecture_correlations.png", dpi=300, bbox_inches='tight')
        fig7.savefig(f"{plot_dir}/architecture_configurations.png", dpi=300, bbox_inches='tight')
        # Save analysis data
    ...
    if save_plots:
        ...
        print("  • performance_analysis.png - Performance distributions and success rates")
