...
class QuantizationObserver(nn.Module):
    ...
    def __init__(self, config: QuantizationConfig):
        ...
        self.register_buffer(
            "num_batches", torch.zeros((), dtype=torch.long), persistent=True
        )
        self.enabled = True
    ...
    def _update_stats(self, x: torch.Tensor):
        ...
        if self.min_val is None or self.max_val is None:
            ...
            if self.config.moving_avg_factor > 0:
                maf = self.config.moving_avg_factor
                self.min_val = (1 - maf) * self.min_val + maf * cur_min
                self.max_val = (1 - maf) * self.max_val + maf * cur_max
            else:
    ...
    def calculate_scale_zero_point(self):
        if self.min_val is None or self.max_val is None:
            raise RuntimeError(
                "Observer has no data. Call _update_stats(...) before calculating qparams."
            )
        cfg = self.config
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        # int8 weights
        self.register_buffer(
            "weight_int", torch.zeros(out_features, in_features, dtype=w_dtype)
        )
        ...
        self.register_buffer(
            "weight_zero_point", torch.zeros(s_shape, dtype=torch.long)
        )
        # bias int32 (static)
        if bias:
            self.register_buffer(
            ...
            self.register_buffer(
                "bias_scale", torch.ones(s_shape)
            )  # should equal input_scale * weight_scale
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_int = torch.clamp(
            torch.round(x / a_s) + a_zp, self.config.qmin, self.config.qmax
        ).to(torch.int8)
        # ----- Center integers -----
    ...
    def from_float(
        ...
        w_obs = QuantizationObserver(
            QuantizationConfig(
                bit_width=cfg.bit_width, symmetric=True, per_channel=cfg.per_channel
            )
        )
        w_obs._update_stats(float_module.weight.data)
        w_scale, w_zp = w_obs.calculate_scale_zero_point()
        if cfg.per_channel:
            w_int = torch.round(float_module.weight / w_scale.view(-1, 1)) + w_zp.view(
                -1, 1
            )
        else:
        ...
        if act_observer is None or act_observer.min_val is None:
            raise ValueError(
                "from_float requires a calibrated activation observer to set input scale/zero_point."
            )
...
class DynamicQuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        # Persistent activation observer (asymmetric per-tensor) with EMA for smoothing
        act_cfg = QuantizationConfig(
            bit_width=self.config.bit_width,
            symmetric=False,
            per_channel=False,
            moving_avg_factor=0.1,  # Smoothing factor to reduce noise
        )
        self.act_observer = QuantizationObserver(act_cfg)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        dev = x.device
        # Update activation stats (with EMA)
        with torch.no_grad():
            self.act_observer._update_stats(x)
        a_scale, a_zp = self.act_observer.calculate_scale_zero_point()
        a_scale = a_scale.to(dev)
        a_zp = a_zp.to(dev)
        x_int = torch.clamp(
            torch.round(x / a_scale) + a_zp, self.config.qmin, self.config.qmax
        ).to(torch.int8)
        Wq = self.weight_int.to(dev)
        w_s = self.weight_scale.to(dev)
        w_zp = self.weight_zero_point.to(dev)
