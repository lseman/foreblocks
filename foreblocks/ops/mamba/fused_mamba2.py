...
if FUSED_MAMBA2_TRITON_AVAILABLE:
    ...
    def _fused_mamba2_fwd_kernel(
        ...
        for ti in range(CHUNK_SIZE):
            ...
            # Note: fused kernel processes per-head; full RMSNorm over d_inner
            # is computed in the standard path. Here we do per-head gating.
...
def fused_mamba2_forward(
    ...
    dt_hidden : [B, T, dt_rank]  — dt source
    ...
    dt_rank = dt_proj_weight.shape[1] if has_dt_proj else dt_hidden.shape[-1]
    ...
    if not can_fuse:
        raise RuntimeError(
            ...
            f"(conv_bias={conv_bias is not None}, B={B is not None}, C={C is not None})"
        )
