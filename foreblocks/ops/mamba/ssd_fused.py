"""Fully-fused SSD forward — three Triton kernel launches.

Reduces 5+ PyTorch kernels to 3 Triton launches with no host-side L/G matrices:

  Kernel 1 (_ssd_cumsum_kernel): parallel over (B*nc*H)
    - dtA = dt*A, cs = cumsum(dtA) within each chunk.

  Kernel 2 (_ssd_intra_kernel): parallel over (B*nc*H)
    - Y_intra[t,p] = sum_{j<=t} exp(cs[t]-cs[j]) * dt[j] * G[t,j] * u[j,p]
    - state_end[p,n] = sum_j exp(cs_last-cs[j]) * dt[j] * B[j,n] * u[j,p]
    Uses dynamic range() to keep compile time fast.

  Kernel 3 (_ssd_scan_kernel): serial over nc chunks, parallel over (B*H)
    - State Passing: S_in[c+1] = exp(cs_last[c]) * S_in[c] + state_end[c]
    - Chunk Scan:    y[t] = Y_intra[t] + exp(cs[t])*(C[t]@S_in) + D*u[t]

Math matches _chunked_ssd_forward_modular (ssd.py) exactly. No tanh/G hacks.
Correct state propagation across all chunks (prior ssd_fused.py was broken).
Default chunk_size=128 per blog recommendation for fused kernels.
"""
...
def _ssd_intra_kernel(
    ...
    for t in range(CS):
        ...
        tl.store(Y_intra_ptr + base_y + t * SHP + p_offs,
                 y_t.to(Y_intra_ptr.dtype.element_ty), mask=p_mask)
    ...
    tl.store(state_end_ptr + base_se + se_off,
...
def _ssd_scan_kernel(
    ...
    if HAS_INIT:
        S = tl.load(s_in_ptr + (pid_b * H + pid_h) * P * N + se_off,
                    mask=se_mask).to(tl.float32)
    ...
    for c in range(nc):
        ...
        for t in range(CS):
            ...
            yi_t = tl.load(Y_intra_ptr + base_yi + t * SHP + p_offs, mask=p_mask).to(tl.float32)
            y_t  = yi_t + tl.exp(cs_t) * tl.sum(S * c_t[None, :], axis=1) + d_v * u_t
            tl.store(y_ptr + base_y + t * SHP + p_offs,
                     y_t.to(y_ptr.dtype.element_ty), mask=p_mask)
...
def fused_ssd_forward(
    ...
    u = u.contiguous(); dt = dt.contiguous()
    ...
    _ssd_scan_kernel[(Bsz, H)](
        Y_intra, state_end, cs_buf,
        u_r, C_r, D, s_in, y_out,
        nc, chunk_size, H, P, N,
        BLOCK_P, BLOCK_N,
        D is not None,
        has_init,
    ...
    if return_final_states:
        ...
        final = s_in.clone()
        for c in range(nc):
            final = torch.exp(cs_last_all[:, c, :])[:, :, None, None] * final + state_end[:, c]
        return y, final
