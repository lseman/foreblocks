"""tilelang flash-attention backward.

On RTX 4090 (Ada) the tilelang backward runs ~1.4-1.5x faster than the Triton
backward at D=128 and is roughly SDPA-parity, so it is preferred when available.

Ported from tilelang's ``example_mha_bwd_bhsd`` with three adaptations required
to plug into custom_att:

* **dtype-parametric** (fp16 and bf16). The upstream example hardcodes float16,
  which segfaults on bf16 inputs.
* **custom ``softmax_scale``** threaded through (upstream hardcodes 1/sqrt(d)).
* the package stores **natural-log** LSE, while this kernel expects the
  **log2-domain** LSE (lse_log2 = lse_nat * log2(e)); the caller converts.

Only the backward kernels are used here; the forward LSE/out come from the
existing Triton/reference forward.
"""
import functools
...
@functools.lru_cache(maxsize=None)
def _build_bwd(batch, heads, seq_len, dim, is_causal, block_M, block_N, scale, dtype_key):
...
def tilelang_flash_bwd(grad_out, q, k, v, out, lse, causal=False, softmax_scale=None):
    ...
    kern = _build_bwd(B, H, N, D, bool(causal), block_M, block_N, float(scale), dtype_key)
