...
The LSE produced is in **log2-domain** (same format that the TileLang backward
kernel expects).  To convert to natural-log: ``lse_nat = lse_log2 * ln(2)``.
"""
import functools
...
def _build_fwd(batch, heads, seq_len, dim, is_causal,
    ...
    def build():
        ...
        def flash_fwd(
            ...
            with T.Kernel(T.ceildiv(seq_len, block_M), heads, batch,
                ...
                T.copy(
                    logsum,
                    lse[bz, by, bx * block_M: (bx + 1) * block_M])
...
def tilelang_flash_fwd(q, k, v, causal=False, softmax_scale=None):
    ...
    n_ctx = q.shape[-2]
    d_head = q.shape[-1]
    softmax_scale = (softmax_scale if softmax_scale is not None
                     else d_head ** -0.5)
    B, H = q.shape[0], q.shape[1]

    block_M, block_N, num_stages, threads = _select_config(
