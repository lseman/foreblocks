...
class HierarchicalBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(level_outputs) > 1:
            ...
            reshaped_levels = stacked_levels.permute(1, 2, 0, 3).reshape(batch_size*seq_len, self.num_levels, hidden_dim)
            ...
            attended_levels = attended_levels.reshape(batch_size, seq_len, self.num_levels, hidden_dim)
...
class NHA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_sequence: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """
        Args:
            x: Input tensor [batch_size, seq_len, input_dim]
