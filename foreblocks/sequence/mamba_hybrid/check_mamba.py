...
def check_ssd_correctness(device):
    ...
    ae, re = _err(fast, ref)
    print(f"  torch fast  vs seq-ref : abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")

    if CHUNKED_SSD_TRITON_AVAILABLE and device == "cuda":
        tri = chunked_ssd_forward_triton(u, dt, A, Bm, Cm, D, chunk_size=cs)
        ae, re = _err(tri, ref)
        print(f"  triton      vs seq-ref : abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")
...
def check_mamba3_adt(device):
    ...
    # forward vs oracle
    y_fast = _chunked_ssd_forward_torch(u, dt, A_dummy, Bm, Cm, D, chunk_size=cs, adt=adt)
    y_ref = _seq_scan_autograd(u, dt, None, Bm, Cm, D, adt=adt)
    ae, re = _err(y_fast, y_ref)
    print(f"  forward (adt) torch vs seq-ref : abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")

    if CHUNKED_SSD_TRITON_AVAILABLE and device == "cuda":
        y_tri = chunked_ssd_forward_triton(
            u.detach(), dt.detach(), A_dummy.detach(),
            Bm.detach(), Cm.detach(), D.detach(), chunk_size=cs, adt=adt.detach(),
        )
        ae, re = _err(y_tri, y_ref)
        print(f"  forward (adt) triton vs seq-ref: abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")

    # backward through the real chunked_ssd_forward autograd Function with adt
    g = torch.randn_like(y_fast)
    y_auto = chunked_ssd_forward(u, dt, A_dummy, Bm, Cm, D, chunk_size=cs, use_triton=False, adt=adt)
    y_auto.backward(g)
    ...
    print(f"  backward du   vs seq-ref : abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-2 else 'FAIL  <-- adt ignored in backward?'}")
    if adt.grad is None:
        print("  d(adt): NO GRAD  <-- adt is not differentiated by the backward!")
    else:
        ae, re = _err(adt.grad, adt2.grad)
        print(f"  d(adt)        vs seq-ref : abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-2 else 'FAIL'}")
...
def check_trapezoid(device):
    ...
    y_fast = _chunked_ssd_forward_torch(u, dt, D, Bm, Cm, D, chunk_size=cs, adt=adt, trap=trap)
    y_ref = _seq_scan_autograd(u, dt, None, Bm, Cm, D, adt=adt, trap=trap)
    ae, re = _err(y_fast, y_ref)
    print(f"  forward (trap) torch  vs seq-ref: abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")

    if CHUNKED_SSD_TRITON_AVAILABLE and device == "cuda":
        from foreblocks.ops.mamba.ssd import chunked_ssd_forward
        with torch.no_grad():
            y_tri = chunked_ssd_forward(u, dt, D, Bm, Cm, D, chunk_size=cs,
                                        use_triton=True, adt=adt, trap=trap)
        ae, re = _err(y_tri, y_ref)
        print(f"  forward (trap) triton vs seq-ref: abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-3 else 'FAIL'}")

    # backward through the two-tap autograd path
    u, dt, Bm, Cm, D, adt, trap = mk(rg=True)
    g = torch.randn(B, T, H, P, device=device)
    from foreblocks.ops.mamba.ssd import chunked_ssd_forward
    y1 = chunked_ssd_forward(u, dt, D, Bm, Cm, D, chunk_size=cs, use_triton=False, adt=adt, trap=trap)
    y1.backward(g)
    t2 = [x.detach().clone().requires_grad_(True) for x in (u, dt, Bm, Cm, D, adt, trap)]
    u2, dt2, Bm2, Cm2, D2, adt2, trap2 = t2
    y2 = _seq_scan_autograd(u2, dt2, None, Bm2, Cm2, D2, adt=adt2, trap=trap2)
    y2.backward(g)
    for n, a, b in [("du", u, u2), ("ddt", dt, dt2), ("dB", Bm, Bm2),
                    ("dC", Cm, Cm2), ("d(adt)", adt, adt2), ("d(trap)", trap, trap2)]:
        if a.grad is None:
            print(f"  {n:8s}: NO GRAD")
            continue
        ae, re = _err(a.grad, b.grad)
        print(f"  {n:8s} vs seq-ref: abs={ae:.2e} rel={re:.2e}  {'OK' if ae < 1e-2 else 'FAIL'}")
...
def check_step(device):
    ...
    T = 16
    for name, blk in [
        ("Mamba2", Mamba2Block(128, use_fused_path=False, use_triton_ssd=False, chunk_size=8)),
...
def check_blocks(device):
    ...
    for name, blk in [
        ("Mamba2Block", Mamba2Block(d_model, use_fused_path=False, use_triton_ssd=False, chunk_size=64)),
        ...
        print(f"  {name:12s}: out {tuple(y.shape)} grad_norm={gnorm:.3e} finite={finite} {'OK' if finite else 'FAIL'}")
...
def check_speed(device):
    ...
    for T in (512, 2048):
        ...
        configs = [
            ("Mamba2 torch", Mamba2Block(d_model, use_fused_path=False, use_triton_ssd=False, chunk_size=256)),
            ("Mamba2 triton", Mamba2Block(d_model, use_fused_path=False, use_triton_ssd=True, chunk_size=256)),
            ("Mamba2 fused", Mamba2Block(d_model, use_fused_path=True, use_triton_ssd=True, chunk_size=256)),
            ("Mamba3 torch", Mamba3Block(d_model, use_triton_ssd=False, chunk_size=256)),
            ("Mamba3 triton", Mamba3Block(d_model, use_triton_ssd=True, chunk_size=256)),
        ]
