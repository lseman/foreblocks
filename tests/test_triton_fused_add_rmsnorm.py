...
def _make_inputs(B, T, D, dtype=torch.float32, device="cuda", requires_grad=True):
    residual = torch.randn(
        B, T, D, device=device, dtype=dtype, requires_grad=requires_grad
    )
    update = torch.randn(
        B, T, D, device=device, dtype=dtype, requires_grad=requires_grad
    )
...
def test_gradcheck_float32():
    ...
    B, T, D = 2, 4, 32
    residual = torch.randn(
        B, T, D, device="cuda", dtype=torch.float32, requires_grad=True
    )
    update = torch.randn(
        B, T, D, device="cuda", dtype=torch.float32, requires_grad=True
    )
...
def test_fallback_for_large_d():
    ...
    norm = RMSNorm(D).cuda()
    out = fused_dropout_add_norm(
        residual, update, norm_layer=norm, p=0.0, training=False
    )
    assert out.shape == residual.shape
