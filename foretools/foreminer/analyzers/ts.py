...
class TimeSeriesAnalyzer(AnalysisStrategy):
    ...
    def analyze(self, data: pd.DataFrame, config: AnalysisConfig) -> dict[str, Any]:
        ...
        numeric_cols = get_numeric_columns(data, exclude_cols=[time_col, target_col])

        # Prepare clean series
        series_map = build_series_map(
            data,
            numeric_cols,
            min_length=self.min_series_length,
            max_count=self.max_series_count,
        )
        ...
        modules = {
            ...
            "change_point_detection": (
                ...
                (data, series_map, config),
            ),
            "motif_discovery": (self._detect_motifs, (series_map, config)),
            "trend_coordination": (
                self._analyze_trend_coordination,
                (series_map, config),
            ),
            "forecasting_readiness": (
                self._assess_forecasting_readiness,
                (series_map, config),
            ),
            "volatility_analysis": (self._analyze_volatility, (series_map, config)),
            "causality_analysis": (
                self._granger_causality_analysis,
                (data, list(series_map.keys()), config),
            ),
        }

        for name, (fn, args) in modules.items():
            value, err = safe_call(fn, *args, default={})
            results[name] = value
            if err:
                module_errors[name] = err

        if module_errors:
            results["module_errors"] = module_errors

        return results

    # --------------------------- SOTA Stationarity Testing ---------------------------
    def _analyze_stationarity(
        self, series_map: dict[str, pd.Series], config: AnalysisConfig
    ) -> pd.DataFrame:
                ...
                if not consensus or stationarity_type == "non_stationary":
                    try:
                        ...
                        if len(diff_series) > 10:
                            diff_adf_stat, diff_adf_p = adfuller(
                                diff_series, autolag="AIC"
                            )[:2]
                            diff_info = {
                ...
                results.append(
                    {
                        ...
                        and stationarity_type == "stationary",
                        "is_stationary": consensus
                        and stationarity_type == "stationary",
                        **diff_info,
                    }
                )
    ...
    def _analyze_temporal_patterns(
        ...
        for col, series in series_map.items():
            ...
            try:
                ...
                trend_strength = (
                    abs(slope * len(series)) / series_std if series_std > 0 else 0
                )
                ...
                if cusum_stat > 1.5:  # Significant break threshold
                    ...
                    cusum_breakpoint = float(
                        np.argmax(np.abs(cumulative_sum)) / len(series)
                    )
                ...
                vol_info.update(
                    {
                        ...
                        "vol_autocorr": float(vol_autocorr)
                        if not np.isnan(vol_autocorr)
                        else 0.0,
                        "volatility_persistent": abs(vol_autocorr) > 0.3
                        if not np.isnan(vol_autocorr)
                        else False,
                    }
                )
        ...
        for col, series in series_map.items():
            ...
            try:
                ...
                results[col] = {
                    ...
                    "seasonal_lags": [
                        lag for lag in recommended_lags if lag in [12, 24, 52]
                    ],
                }
    ...
    def _test_seasonality(
        ...
        for col in list(series_map.keys())[:10]:  # Limit for performance
            try:
                ...
                try:
                    dow_groups = [
                        series[series.index.dayofweek == d].values for d in range(7)
                    ]
                ...
                try:
                    frequencies, power = welch(
                        series.values, nperseg=min(len(series) // 4, 256)
                    )
            ...
            try:
                ...
                try:
                    ...
                    if n >= 24:
                        ...
                        stl = STL(
                            series, seasonal=seasonal_param, period=period, robust=True
                        ).fit()
                        seasonal_var = np.var(stl.seasonal)
                        total_var = np.var(series)
                        seasonality_score = (
                            min((seasonal_var / total_var) * 2, 1.0)
                            if total_var > 0
                            else 0
                        )
        ...
        for col, series in series_map.items():
            ...
            try:
                ...
                try:
                    ...
                    arch_stat, arch_p = het_arch(
                        returns, nlags=min(5, len(returns) // 20)
                    )[:2]
                ...
                for j in range(i + 1, len(cols_subset)):
                    ...
                    try:
                        ...
                        gc_result_12 = grangercausalitytests(
                            df[[var2, var1]].values, maxlag=max_lag, verbose=False
                        )

                        # Extract best lag and p-value
                        p_values_12 = {
                            lag: gc_result_12[lag][0]["ssr_ftest"][1]
                            for lag in gc_result_12
                        }
                    ...
                    try:
                        ...
                        gc_result_21 = grangercausalitytests(
                            df[[var1, var2]].values, maxlag=max_lag, verbose=False
                        )

                        # Extract best lag and p-value
                        p_values_21 = {
                            lag: gc_result_21[lag][0]["ssr_ftest"][1]
                            for lag in gc_result_21
                        }
                    ...
                    pair_results["bidirectional"] = (
                        pair_results["var1_causes_var2"]["significant"]
                        and pair_results["var2_causes_var1"]["significant"]
                    )
    ...
    def _detect_motifs(
        self, series_map: dict[str, pd.Series], config: Any
    ) -> dict[str, Any]:
        """
        Detect recurring motifs using a vectorized Lite Matrix Profile approach.
        """
        motifs_results = {}
        window_size = getattr(config, "motif_window", 20)
        ...
        for col, s in series_map.items():
            ...
            # Normalize for distance comparison
            denom = values.std() + 1e-8
            v_norm = (values - values.mean()) / denom

            # Simple Matrix Profile approximation (Euclidean Distance)
            n = len(v_norm)
            m = window_size

            # Use rolling window views for vectorization
            # Note: windows will have shape (n - m + 1, m)
            windows = np.lib.stride_tricks.sliding_window_view(v_norm, m)

            # Subsample windows if series is too long for O(N^2)
            # We want to keep it fast for mining
            step = 1 if n < 500 else max(1, n // 200)
            target_indices = np.arange(0, len(windows), step)

            min_dist = np.full(len(windows), np.inf)
            best_match = np.zeros(len(windows), dtype=int)

            for i in target_indices:
                w = windows[i]
                # Distances to all other non-overlapping windows
                # Exclusion zone: i +/- m/2
                mask = np.abs(np.arange(len(windows)) - i) > m // 2
                if not np.any(mask):
                    continue

                # Vectorized Euclidean
                dists = np.sqrt(np.sum((windows[mask] - w) ** 2, axis=1))

                # Find best non-overlapping match for window i
                if len(dists) > 0:
                    min_idx = np.argmin(dists)
                    actual_min_idx = np.where(mask)[0][min_idx]
                    min_dist[i] = dists[min_idx]
                    best_match[i] = actual_min_idx

            # Top motifs are indices with smallest min_dist
            potential_motifs = np.argsort(min_dist)
            found = []
            seen_indices = set()

            for idx in potential_motifs:
                ...
                if any(abs(idx - f[0]) < m for f in found) or any(
                    abs(match_idx - f[0]) < m for f in found
                ):
                    continue

                found.append((int(idx), int(match_idx), float(min_dist[idx])))

            if found:
                motifs_results[col] = {
                    "top_motifs": found,
                    "window_size": m,
                    "avg_dist": float(np.mean([f[2] for f in found])),
                }

        return motifs_results

    def _analyze_trend_coordination(
        self, series_map: dict[str, pd.Series], config: Any
    ) -> dict[str, Any]:
        """
        Detect simultaneous trend shifts or seasonal phase alignment across features.
        """
        coordination = {"coordinated_trends": [], "phase_alignments": []}
        cols = list(series_map.keys())
        if len(cols) < 2:
            return coordination

        # 1. Trend Direction Correlation
        trends = {}
        for col, s in series_map.items():
            # Rolling slope as trend proxy
            v = s.values
            if len(v) < 20:
                continue
            try:
                slopes = np.polyfit(np.arange(len(v))[-20:], v[-20:], 1)[0]
                trends[col] = np.sign(slopes)
            except:
                continue

        # 2. Sequential Similarity (Phase Alignment)
        for i in range(len(cols)):
            for j in range(i + 1, len(cols)):
                c1, c2 = cols[i], cols[j]
                s1, s2 = series_map[c1], series_map[c2]

                # Normalize
                v1 = (s1.values - s1.mean()) / (s1.std() + 1e-8)
                v2 = (s2.values - s2.mean()) / (s2.std() + 1e-8)

                # Cross-correlation to find phase shift
                corr = np.correlate(v1, v2, mode="same")
                max_idx = np.argmax(corr)
                lag = max_idx - (len(corr) // 2)
                strength = float(corr[max_idx]) / len(v1)

                if abs(strength) > 0.6:
                    coordination["phase_alignments"].append(
                        {
                            "pair": (c1, c2),
                            "correlation": strength,
                            "lag": int(lag),
                            "interpretation": "Highly synchronized"
                            if abs(lag) < 3
                            else f"{c1} leads {c2}"
                            if lag < 0
                            else f"{c2} leads {c1}",
