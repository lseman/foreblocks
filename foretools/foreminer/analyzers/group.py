...
class CategoricalGroupAnalyzer(AnalysisStrategy):
    ...
    def _prepare_group_data(
        ...
        if len(valid_groups) < 2:
            raise ValueError(
                f"Need at least 2 groups with minimum {self.min_group_size} samples"
            )
    ...
    def _smart_assumption_testing(
        self, grouped_data: dict[str, pd.DataFrame], numeric_col: str
    ) -> dict[str, Any]:
        ...
        if len(group_arrays) == 2:
            ...
            try:
                ...
                results["mann_whitney"] = {
                    ...
                    "effect_size": self._interpret_effect_size(
                        abs(rank_biserial), "rank_biserial"
                    ),
                    "groups_compared": group_names,
                ...
                if HAS_BOOTSTRAP:
                    ...
                    for _ in range(self.bootstrap_iterations):
                        resampled = np.random.choice(
                            pooled_data, size=len(pooled_data), replace=True
                        )
    ...
    def _categorical_association_tests(
        self, data: pd.DataFrame, cat_col1: str, cat_col2: str
    ) -> dict[str, Any]:
    ...
    def _generate_recommendations(
        ...
        for test_name, result in test_results.items():
            if (
                ...
                and result.get("significant", False)
        ...
        if significant_tests:
            ...
            recommendations.append(
                f"🎯 **Significant difference detected** using {method} (p = {p_val:.6f})"
            )
            ...
            if "bootstrap" in best_test_name:
                recommendations.append(
                    "✅ Result confirmed by bootstrap resampling - highly reliable"
                )
            elif "welch" in best_test_name:
                recommendations.append(
                    "🔧 Welch's method used - robust to unequal variances"
                )
        else:
            recommendations.append(
        ...
        if not group_info.get("balanced_groups", True):
            recommendations.append(
                "⚖️ Unbalanced groups detected - robust methods recommended"
            )
        ...
        if min_size < 10:
            recommendations.append(
                "📈 Very small groups - consider collecting more data"
            )
        elif min_size >= 100:
            recommendations.append(
    ...
    def analyze(
        ...
        try:
            ...
            if not self._is_suitable_categorical_column(data, categorical_col):
                raise ValueError(
                    f"Column '{categorical_col}' not suitable for group analysis"
                )
            ...
            # Prepare data
            grouped_data, group_info = self._prepare_group_data(
                data, categorical_col, target_cols
            )
            ...
            for target_col in target_cols:
                ...
                test_results = self._run_statistical_tests(
                    grouped_data, target_col, assumptions
                )
                ...
                recommendations = self._generate_recommendations(
                    test_results, assumptions, group_info
                )
            ...
            for cat_col in categorical_cols:
                if cat_col != categorical_col and cat_col not in target_cols:
                    try:
                        assoc_results = self._categorical_association_tests(
                            data, categorical_col, cat_col
                        )
                        if assoc_results and "error" not in assoc_results:
                            analysis_results["categorical_associations"][cat_col] = (
                                assoc_results
                            )
                    except Exception:
                        continue
            ...
            analysis_results["summary"] = {
                ...
                "categorical_associations_tested": len(
                    analysis_results["categorical_associations"]
                ),
