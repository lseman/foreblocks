...
class GraphAnalyzer(AnalysisStrategy):
    ...
    # --------------------------- Graph Construction ---------------------------
    def _construct_correlation_network(
    ...
    def _construct_knn_network(self, data: pd.DataFrame, k: int = None) -> nx.Graph:
        ...
        data_scaled = scaler.fit_transform(
            data.T
        )  # Transpose for variable relationships
    ...
    def _construct_threshold_network(
        self, data: pd.DataFrame, method: str = "euclidean"
    ) -> nx.Graph:
    ...
    def _construct_mutual_information_network(self, data: pd.DataFrame) -> nx.Graph:
        ...
        threshold = (
            np.percentile(mi_matrix[mi_matrix > 0], 70) if (mi_matrix > 0).any() else 0
        )
    ...
    def _construct_sample_network(
        self, data: pd.DataFrame, method: str = "correlation"
    ) -> nx.Graph:
    ...
    def _analyze_network_topology(self, G: nx.Graph) -> dict[str, Any]:
        ...
        if nx.is_connected(G):
            ...
            if largest_component.number_of_edges() > 0:
                ...
                topology["largest_component_avg_path_length"] = (
        ...
        try:
            ...
            if m > 0 and n > 1:
                ...
                random_path_length = (
                    np.log(n) / np.log(2 * m / n) if m > 0 else float("inf")
                )
                ...
                if random_clustering > 0 and random_path_length < float("inf"):
                    small_world_sigma = (actual_clustering / random_clustering) / (
                        actual_path_length / random_path_length
                    )
    ...
    def _detect_communities(self, G: nx.Graph) -> dict[str, Any]:
        ...
        try:
            if (
                G.number_of_edges() > 0 and G.number_of_nodes() <= 100
            ...
            if G.number_of_nodes() >= 4:
                ...
                for n_clusters in range(2, min(8, G.number_of_nodes() // 2)):
                    try:
                        ...
                        partition = {
                            node: int(labels[i]) for i, node in enumerate(G.nodes())
                        }
        ...
        try:
            if G.number_of_edges() > 0:
                ...
                centralities["eigenvector"] = {
                    ...
                    "top_nodes": sorted(
                        eigenvector_cent.items(), key=lambda x: x[1], reverse=True
                    )[:5],
    ...
    def _network_embedding_analysis(self, G: nx.Graph) -> dict[str, Any]:
        ...
        if HAS_NODE2VEC and G.number_of_nodes() >= 4:
            try:
                ...
                node2vec = Node2Vec(
                    G, dimensions=64, walk_length=30, num_walks=200, workers=1
                )
                    ...
                    for i in range(len(nodes)):
                        for j in range(i + 1, len(nodes)):
                            similar_pairs.append(
                                (nodes[i], nodes[j], similarities[i, j])
                            )
        ...
        try:
            ...
            if len(numeric_data) > self.sample_threshold:
                sample_data = numeric_data.sample(
                    self.sample_threshold, random_state=42
                )
                was_subsampled = True
            else:
            ...
            for gtype in graph_types_to_try:
                try:
                    ...
                    if (
                        gtype == graph_types_to_try[0]
                        or len(analysis_results["graphs"]) == 1
                    ):
    ...
    def _generate_graph_summary_and_recommendations(
        ...
        summary = {
            ...
            "successful_constructions": len(
                [g for g in results["graphs"].values() if "error" not in g]
            ),
        ...
        if best_graph:
            ...
            recommendations.append(
                f"🏆 **Best graph structure**: {best_graph.replace('_', ' ').title()} Network"
            )

            # Connectivity insights
            if best_topology.get("is_connected", False):
                recommendations.append(
                    "🔗 **Highly connected network** - variables form a cohesive system"
                )
                ...
                if avg_path <= 3:
                    recommendations.append(
                        "⚡ **Short path lengths** - information flows efficiently through network"
                    )
                elif avg_path > 5:
                    recommendations.append(
                        "🌉 **Long path lengths** - some variables are distantly related"
                    )
            else:
                n_components = best_topology.get("n_components", 0)
                if n_components > 1:
                    recommendations.append(
                        f"📊 **Disconnected network** - {n_components} separate variable clusters detected"
                    )
                    recommendations.append(
                        "💡 Consider analyzing clusters independently or finding bridging variables"
                    )
            ...
            if clustering > 0.5:
                recommendations.append(
                    "🔘 **High clustering** - variables form tight-knit groups"
                )
            elif clustering > 0.3:
                recommendations.append(
                    "🔗 **Moderate clustering** - some variable groupings detected"
                )
            else:
                recommendations.append(
                    "📈 **Low clustering** - variables have sparse local connections"
                )

            # Small world insights
            if best_topology.get("is_small_world", False):
                recommendations.append(
                    "🌍 **Small-world network** - efficient global connectivity with local clustering"
                )
                recommendations.append(
                    "🚀 Ideal for feature engineering and dimensionality reduction"
                )

            # Community insights
            community_methods = [
                method
                for method in best_communities.keys()
                if "error" not in best_communities[method]
            ]
            if community_methods:
                ...
                if best_community_method and best_modularity > 0.3:
                    n_communities = best_communities[best_community_method][
                        "n_communities"
                    ]
                    recommendations.append(
                        f"🏘️ **{n_communities} communities detected** using {best_community_method.replace('_', ' ').title()}"
                    )
                    recommendations.append(
                        f"📈 High modularity ({best_modularity:.3f}) indicates strong community structure"
                    )
                elif best_modularity > 0:
                    recommendations.append(
                        "🔍 **Weak community structure** - variables are somewhat grouped"
                    )
                else:
                    recommendations.append(
                        "🌐 **No clear communities** - variables are uniformly connected"
                    )

            # Centrality insights
            if (
                "degree" in best_centralities
                and "error" not in best_centralities["degree"]
            ):
                top_degree_nodes = best_centralities["degree"]["top_nodes"]
                if top_degree_nodes:
                    top_node = top_degree_nodes[0][0]
                    recommendations.append(
                        f"⭐ **Most connected variable**: {top_node}"
                    )

            if (
                "pagerank" in best_centralities
                and "error" not in best_centralities["pagerank"]
            ):
                top_pagerank_nodes = best_centralities["pagerank"]["top_nodes"]
                if top_pagerank_nodes:
                    top_node = top_pagerank_nodes[0][0]
                    recommendations.append(
                        f"👑 **Most influential variable**: {top_node}"
                    )
            ...
            if (
                embeddings
                and "node2vec" in embeddings
                and "error" not in embeddings["node2vec"]
            ):
                recommendations.append(
                    "🧠 **Node embeddings available** - use for advanced ML feature engineering"
                )
                ...
                if similar_pairs:
                    ...
                    recommendations.append(
                        f"🔗 **Most similar variables**: {most_similar[0]} ↔ {most_similar[1]} (similarity: {most_similar[2]:.3f})"
                    )
        ...
        if data_info["was_subsampled"]:
            recommendations.append(
                "📊 **Large dataset subsampled** - results are representative but may miss rare patterns"
            )

        if len(data_info["constant_columns_removed"]) > 0:
            recommendations.append(
                f"🚫 **{len(data_info['constant_columns_removed'])} constant columns removed** - no network information"
            )
        ...
        if successful_graphs == total_attempted:
            recommendations.append(
                "✅ **All graph construction methods successful** - robust network structure"
            )
        elif successful_graphs > 0:
            recommendations.append(
                f"⚠️ **{successful_graphs}/{total_attempted} graph methods successful** - some approaches failed"
            )
        else:
            recommendations.append(
                "❌ **No successful graph constructions** - data may lack network structure"
            )
        ...
        if best_graph:
            ...
            if best_nodes >= 10 and best_edges >= 20:
                recommendations.append(
                    "🔬 **Rich network structure** - suitable for advanced graph ML algorithms"
                )

            if best_topology.get("density", 0) > 0.5:
                recommendations.append(
                    "🕸️ **Dense network** - consider edge pruning or sparsification"
                )
            elif best_topology.get("density", 0) < 0.1:
                recommendations.append(
                    "🌿 **Sparse network** - may benefit from relaxed connection thresholds"
                )

        results["recommendations"] = recommendations[:8]  # Limit to top recommendations

    def get_network_visualization_data(
        self, results: dict[str, Any], graph_type: str = None
    ) -> dict[str, Any]:
        """
        ...
        for node in G.nodes():
            node_info = {
                "id": node,
                "label": node,
                "size": degree_cent.get(node, 0.1) * 100
        ...
        return {
            ...
            "n_edges": len(edges),
            "layout": "spring",
            "communities": len(set(community_assignment.values()))
            if community_assignment
            else 1,
        }
