...
class ClusterAnalyzer(AnalysisStrategy):
    ...
    def _modern_optimal_k(self, X: np.ndarray, max_k: int = 15) -> dict[str, Any]:
        ...
        if len(methods) == 2:
            ...
            optimal_k = int(
                np.round(0.6 * methods["silhouette"] + 0.4 * methods["bic_selection"])
            )
    ...
    def _silhouette_analysis(self, X: np.ndarray, max_k: int) -> int:
        ...
        for k in range(2, max_k + 1):
            ...
            for _ in range(3):
                kmeans = KMeans(
                    n_clusters=k, random_state=np.random.randint(1000), n_init=5
                )
    ...
    def _advanced_kmeans(
        self, X: np.ndarray, config: AnalysisConfig, optimal_k: int
    ) -> dict[str, Any]:
    ...
    def _modern_spectral_clustering(
        self, X: np.ndarray, config: AnalysisConfig, optimal_k: int
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if n_samples < 500:
                ...
                affinity = kneighbors_graph(
                    X, n_neighbors=k_neighbors, mode="connectivity"
                )
    ...
    def _modern_density_clustering(
    ...
    def _ensemble_clustering(
        ...
        for name, result in all_results.items():
            if isinstance(result, dict) and "labels" in result:
                ...
                if len(labels) == len(X):
                    ...
                    noise_ratio = (
                        np.sum(labels == -1) / len(labels) if -1 in labels else 0
                    )
        ...
        try:
            ...
            spectral = SpectralClustering(
                n_clusters=n_clusters_est, affinity="precomputed", random_state=42
            )
    ...
    def _majority_vote_ensemble(
        self, valid_results: dict[str, np.ndarray], X: np.ndarray
    ) -> dict[str, Any]:
                ...
                if n_clusters > 1:
                    ...
                    if n_noise > 0:
                        ...
                        if (
                            ...
                            and len(np.unique(labels[non_noise_mask])) > 1
                        ):
                            X_clean = X[non_noise_mask]
                            labels_clean = labels[non_noise_mask]

                            eval_result["silhouette_score"] = float(
                                silhouette_score(X_clean, labels_clean)
                            )
                            eval_result["calinski_harabasz_score"] = float(
                                calinski_harabasz_score(X_clean, labels_clean)
                            )
                            eval_result["davies_bouldin_score"] = float(
                ...
                if n_clusters > 1:
                    ...
                    if len(non_noise_labels) > 0:
                        ...
                        if len(cluster_sizes) > 1:
                            eval_result["cluster_balance"] = float(
                                1
                                - (
                                    np.std(cluster_sizes)
                                    / (np.mean(cluster_sizes) + 1e-10)
                                )
                            )

                # Method-specific metrics
                if "consensus_strength" in result:
                    eval_result["consensus_strength"] = float(
    ...
    def _generate_recommendations(
        ...
        if method_scores:
            ...
            recommendations.append(
                f"Best performing method: {best_method.upper().replace('_', ' ')} (score: {best_score:.3f})"
            )
            ...
            if "density" in method_type:
                ...
                if noise_ratio > 0.2:
                    recommendations.append(
                        "High noise detected - data may have outliers or natural noise structure"
                    )
                else:
                    recommendations.append(
                        "Density-based clustering found clear cluster boundaries"
                    )
                ...
                if certainty > 0.8:
                    ...
                    recommendations.append(
                        "Low certainty suggests overlapping or fuzzy cluster boundaries"
                    )
                ...
                if consensus > 0.7:
                    recommendations.append(
                        "Strong consensus across methods validates cluster structure"
                    )
                else:
                    recommendations.append(
                ...
                recommendations.append(
                    "Spectral clustering success suggests non-convex cluster shapes"
                )
        ...
        if confidence == "high":
            recommendations.append(
                f"Strong evidence for {optimal_k} clusters across multiple methods"
            )
        elif confidence == "medium":
            recommendations.append(
                f"Moderate evidence for {optimal_k} clusters - consider validation"
            )
        else:
            recommendations.append(
        ...
        if preprocessing_info.get("pca_applied", False):
            ...
            recommendations.append(
                f"Dimensionality reduced via PCA ({variance_explained:.1%} variance retained)"
            )
    ...
    def analyze(self, data: pd.DataFrame, config: AnalysisConfig) -> dict[str, Any]:
        ...
        try:
            ...
            optimal_k_info = self._modern_optimal_k(
                X, getattr(config, "max_clusters", 15)
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "successful_methods": len(
                        [r for r in clustering_results.values() if "labels" in r]
                    ),
                    "best_method": (
                        max(
                            evaluations.keys(),
