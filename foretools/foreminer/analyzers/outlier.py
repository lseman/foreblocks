...
class OutlierAnalyzer(AnalysisStrategy):
    ...
    def _lightning_preprocessing(self, data: pd.DataFrame, config: AnalysisConfig):
        ...
        numeric_no_const, keep_cols = drop_constant_numeric(numeric)
        if not keep_cols:
            raise ValueError("All numeric columns are constant.")
        if len(keep_cols) < n_features:
            numeric = numeric_no_const
    ...
    def analyze(self, data: pd.DataFrame, config: AnalysisConfig) -> Dict[str, Any]:
        try:
            ...
            for name, fn in method_calls.items():
                res, err = safe_call(fn, X, config)
                methods[name] = res
                if err:
            ...
            # Ensemble
            ens, ens_err = safe_call(self._ensemble, all_results, X, config)
            if ens:
                all_results["ensemble"] = ens
            elif ens_err:
                method_errors["ensemble"] = ens_err
