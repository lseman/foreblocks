...
class RFECVConfig:
    ...
    feature_importance_method: str = 'auto'  # 'auto', 'coef', 'feature_importances', 'permutation'
...
class AdvancedRFECV(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self,
        estimator=None,
        config: Optional[RFECVConfig] = None,
        **kwargs
    ):
    ...
    def _extract_feature_importance(self, estimator, X: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    def _calculate_step_size(self, n_features: int) -> int:
        ...
        if isinstance(self.config.step, int):
            return min(self.config.step, n_features - self.config.min_features_to_select)
        else:  # float
            ...
            if self.config.use_ensemble:
                ...
                for name, estimator in estimators.items():
                    ...
                    if self._task_type == 'classification':
                        ...
                        pred = est_clone.predict(X_val_subset)
                        score = -mean_squared_error(y_val, pred)  # Negative MSE for maximization
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y: Union[pd.Series, np.ndarray]) -> 'AdvancedRFECV':
        """
        ...
        if self.config.verbose > 0:
            print(f"🔄 Starting Advanced RFECV...")
        ...
        while np.sum(feature_mask) > self.config.min_features_to_select:
            ...
            if current_n_features > max_features:
                ...
                if self.config.verbose > 0:
                    print(f"Iteration {iteration + 1}: {current_n_features} -> {np.sum(feature_mask)} features (fast mode)")
            ...
            if cv_score > best_score + self.config.improvement_threshold:
                ...
                if self.config.verbose > 0:
                    print(f"✅ New best score: {best_score:.4f} with {best_n_features} features")
            else:
                patience_counter += 1
            ...
            if patience_counter >= self.config.patience:
                if self.config.verbose > 0:
                    print(f"🛑 Early stopping: No improvement for {self.config.patience} iterations")
                break
        ...
        if self.config.verbose > 0:
            ...
            if self._feature_names:
                ...
                print(f"Feature names: {selected_names[:10]}{'...' if len(selected_names) > 10 else ''}")
    ...
    def get_feature_ranking(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({
            'feature': self._feature_names or [f"feature_{i}" for i in range(len(self.ranking_))],
    ...
    def plot_cv_scores(self, figsize: Tuple[int, int] = (12, 6)):
        ...
        ax1.axvline(x=self.n_features_, color='red', linestyle='--', 
        ...
        if self._feature_names:
            ...
            bars = ax2.barh(range(len(top_features)), top_importance, 
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'feature_reduction_ratio': (n_features_list[0] - self.n_features_) / n_features_list[0] if n_features_list else 0,
...
class EnhancedFeatureSelector:
    """Enhanced Feature Selector with RFECV integration."""
    
    def __init__(self, config, use_rfecv: bool = False, rfecv_params: Optional[Dict] = None):
        self.config = config
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "EnhancedFeatureSelector":
        ...
        if self.use_rfecv:
            ...
            # Set up RFECV parameters
            rfecv_config = RFECVConfig(
                step=0.1,
                cv=5,
                min_features_to_select=max(1, int(len(X.columns) * 0.05)),  # At least 5% of features
                max_features_to_select=min(100, len(X.columns)),  # Cap at 100 features
                verbose=1,
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.rfecv_selector_ is not None:
            return self.rfecv_selector_.transform(X)
        else:
    ...
    def get_rfecv_results(self) -> Optional[pd.DataFrame]:
        """Get detailed RFECV results."""
        if self.rfecv_selector_ is not None:
            return self.rfecv_selector_.get_feature_ranking()
        return None
    
    def plot_rfecv_results(self, **kwargs):
        """Plot RFECV results."""
        if self.rfecv_selector_ is not None:
            self.rfecv_selector_.plot_cv_scores(**kwargs)
        else:
            print("RFECV was not used in this selector.")
