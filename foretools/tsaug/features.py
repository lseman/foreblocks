...
def _compute_features(x: torch.Tensor) -> torch.Tensor:
    ...
    feats.append(
        ((x - mean.unsqueeze(1)) / std.unsqueeze(1)).pow(3).mean(dim=1)
    )  # 3: skewness
    feats.append(
        ((x - mean.unsqueeze(1)) / std.unsqueeze(1)).pow(4).mean(dim=1)
    )  # 4: kurtosis
...
def _first_ac_crossing(x_centered: torch.Tensor, var: torch.Tensor) -> torch.Tensor:
    ...
    for lag in range(1, max_lag):
        ...
        result = torch.where(
            crossed & (result == max_lag),
            torch.tensor(float(lag), device=result.device),
            result,
        )
