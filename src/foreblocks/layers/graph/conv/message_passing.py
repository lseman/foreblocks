...
def _scatter_add(src: Tensor, index: Tensor, dim_size: int) -> Tensor:
    ...
    # Try Triton scatter-add
    if HAS_TRITON and src.is_cuda and index.is_cuda:
        B, T, E, F = src.shape
        assert index.shape == (E,)
        return triton_scatter_add_op(src, index, dim_size, B, T, F)

    # Fallback to torch_scatter or PyTorch scatter_add
...
class CachedNorm(nn.Module):
    def __init__(self):
        super().__init__()
        self._last_edge_index: Tensor | None = None
        self._last_edge_weight: Tensor | None = None
...
def _edge_index_spmm(
    ...
    # Try Triton fused gather-scatter-add
    if HAS_TRITON and x.is_cuda and src.is_cuda and dst.is_cuda:
        return triton_message_passing_op(x, src, dst, edge_weight)
...
class GraphConvBase:
    def __init__(
        ...
        *,
        ...
        # MessagePassing handles aggr in its __init__ via MRO
        super().__init__()
        self.aggr: AggType = aggr
