...
if HAS_TRITON:
    ...
    def _swiglu_gate_backward(
        ...
        a = tl.load(
            A + batch * stride_ab + time * stride_at + offs * stride_ad,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        b = tl.load(
            B + batch * stride_bb + time * stride_bt + offs * stride_bd,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        dy = tl.load(
            DY + batch * stride_yb + time * stride_yt + offs * stride_yd,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        ...
        g_base = batch * stride_gb + time * stride_gt + offs * stride_gd
        tl.store(DA + g_base, da, mask=mask)
        tl.store(DB + g_base, db, mask=mask)
    ...
    class TritonSwiGLUGate(torch.autograd.Function):
        ...
        def forward(ctx, a: torch.Tensor, b: torch.Tensor):
            ...
            B, T, Dh = a.shape
            # Kernel reads via explicit strides, so strided views (e.g. the
            # two halves of a fused gate_up projection) need no .contiguous().
            y = torch.empty((B, T, Dh), device=a.device, dtype=a.dtype)
        ...
        def backward(ctx, dy):
            ...
            B, T, Dh = a.shape
            da = torch.empty((B, T, Dh), device=a.device, dtype=a.dtype)
            db = torch.empty((B, T, Dh), device=b.device, dtype=b.dtype)
            BLOCK = triton.next_power_of_2(Dh)
            num_warps = 2 if BLOCK <= 64 else (4 if BLOCK <= 512 else 8)
...
def swiglu_gate(a, b):
    if (
        ...
        and a.is_cuda
        and b.is_cuda
        and _swiglu_row_fits(a.shape[-1], a.element_size())
    ):
...
def grouped_mlp_swiglu(
    ...
    if (
        ...
        and GU.is_cuda
        and not torch.jit.is_scripting()
        and _swiglu_row_fits(H, GU.element_size())
    ):
        # Strided views into the fused [S, 2H] projection — no copies; the
        # gate kernel reads via explicit strides.
        G_3d = GU[:, :H].unsqueeze(0)
        U_3d = GU[:, H:].unsqueeze(0)
