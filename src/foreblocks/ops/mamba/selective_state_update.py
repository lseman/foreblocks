...
if SELECTIVE_STATE_UPDATE_TRITON_AVAILABLE:
    ...
    def _selective_scan_update_kernel(
        ...
        # dt is a per-head scalar ([B, H]); pointer already advanced to (b, h)
        ...
        # Load D if present ([H, P] — offset to this head's row)
        if HAS_D:
            D = tl.load(
        ...
        dB = B[None, :] * dt  # dt is a per-head scalar
...
def selective_state_update(
    ...
    # Normalize: x/z are (B, H, P); dt is (B, H) — one scalar per head
    if x.ndim == 2:
        x = x.unsqueeze(1)
    if dt.ndim == 1:
        dt = dt.unsqueeze(1)
    if dt.ndim != 2:
        raise ValueError(f"dt must be [B, H], got shape {tuple(dt.shape)}")
...
def _selective_state_update_pytorch(
    ...
    # Broadcast B, C: [B, G, N] → [B, H, 1, N] (each group serves H/G heads)
    G = B.shape[1]
    B_exp = B.repeat_interleave(H // G, dim=1).view(Bsz, H, 1, N)
    C_exp = C.repeat_interleave(H // G, dim=1).view(Bsz, H, 1, N)
