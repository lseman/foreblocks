						...
						color:
							(point.run.status ?? "").toUpperCase() === "FAILED"
								...
								: selectedSet.has(point.run.run_id)
									...
									: chartTheme.blue,
						opacity:
							selectedSet.size && !selectedSet.has(point.run.run_id)
								? 0.32
								: 0.84,
					},
				})),
		...
		series: [
			{
				...
				data: filteredRuns
					.slice(0, 80)
					.map((run, rowIdx) => {
						const values = parallelActiveDims.map((dim) =>
							parallelDimensionValue(run, dim),
						);
		...
		series: [
			{
				...
				label: {
					...
					formatter: (raw: { data?: { value?: number } }) =>
						formatMetricValue(raw.data?.value),
				},
				data: rows.map((row) => ({
					value: row.score,
					name: row.key,
	...
	return {
		...
		tooltip: {
			...
			formatter: (raw) => {
				...
				const header =
					params[0] && "axisValueLabel" in params[0]
				...
				const rows = params
					.map((entry) => {
						...
						const value =
							"data" in entry && Array.isArray(entry.data)
								? entry.data[1]
								: undefined;
						const color =
							"color" in entry
								? String(entry.color ?? chartTheme.blue)
								: chartTheme.blue;
...
export function buildRunDetailChartOption(args: {
	...
	return {
		...
		grid: { top: 22, right: 24, bottom: 64, left: 58 },
		tooltip: {
			trigger: "axis",
			backgroundColor: chartTheme.tooltipBg,
			borderColor: chartTheme.tooltipBorder,
			...
			formatter: (raw) => {
				...
				const header =
					params[0] && "axisValueLabel" in params[0]
						...
						: "";
				const value =
					params[0] && "data" in params[0] && Array.isArray(params[0].data)
						? params[0].data[1]
						: undefined;
				return [
		...
		dataZoom: [
			{
				...
				fillerColor: chartTheme.accentSoft,
				handleStyle: {
					color: chartTheme.accent,
				},
