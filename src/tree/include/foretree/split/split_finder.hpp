...
public:
    int max_selected_categories = 8; // soft cap in ranked-prefix search
    ...
    splitx::Candidate best_categorical_partition(const splitx::SplitContext& ctx) const {
        using namespace splitx;
        Candidate best;
        best.kind = SplitKind::CategoricalPartition;
        best.gain = NEG_INF;
        if (!ctx.G || !ctx.H || !ctx.C || ctx.P <= 0)
            return best;
        const auto& G = *ctx.G;
        ...
        ctx.for_each_active_feature([&](int f) {
            if (!ctx.is_categorical_feature(f))
                return;
            const int finite_bins = ctx.get_feature_bins(f);
            if (finite_bins <= 1)
                return;
            ...
            for (int t = 0; t < finite_bins; ++t) {
                ...
                if (off >= G.size() || off >= H.size() || off >= C.size())
                    continue;
            ...
            auto maybe_commit = [&](const std::vector<int>& left_bins, double GL, double HL, int CL) {
                ...
                best.gain = gain;
                best.feat = f;
                best.categorical_left_bins = std::move(normalized);
                best.miss_left = miss_left_local;
                best.categorical_missing_child = miss_left_local ? 0 : 1;
            };
            ...
            if (use_exhaustive_search && non_empty <= std::max(2, max_exhaustive_cardinality) &&
                ...
                for (uint64_t mask = 1; mask + 1 < full; ++mask) {
                    // Skip mirrored complements by forcing the first category
                    // to stay on the left.
                    if ((mask & uint64_t{1}) == 0)
                        continue;
            ...
            // 2) Ordered scan (CatBoost-style): sort categories by smoothed
            // statistic and scan prefixes.
            if (use_ordered_scan) {
                ...
                std::sort(ord.begin(), ord.end(), [&](int ia, int ib) {
                    ...
                    if (a.ordered_key != b.ordered_key)
                        return a.ordered_key < b.ordered_key;
                    return a.bin < b.bin;
                });
            ...
            // 3) Ranked multi-candidate scan: prefixes over |stat|-ranked
            // categories.
            if (use_ranked_prefix_scan) {
                ...
                const int prefix_cap = std::min({non_empty - 1, std::max(1, max_ranked_prefix_candidates),
                                                 std::max(1, max_selected_categories - 1)});
                double GL = 0.0, HL = 0.0;
                int CL = 0;
                std::vector<int> left_bins;
            ...
            // 4) Greedy merge on ordered categories for medium/high
            // cardinality.
            if (use_greedy_merge && non_empty >= std::max(2, greedy_merge_min_cardinality) &&
                ...
                while (static_cast<int>(groups.size()) > target_groups) {
                    ...
                    for (int i = 0; i + 1 < static_cast<int>(groups.size()); ++i) {
                        ...
                        // Ward-like adjacent merge cost: merge most similar
                        // neighbors first.
...
struct PairInteractionConfig {
    int max_features = 8;
    int interaction_bins = 8;
    int min_node_rows = 256;
    double complexity_penalty = 0.0;
    double axis_guard_factor = 1.02;
};

// Genuine nonlinear pair split. Each candidate assigns the four quadrants
// formed by (xa <= ta, xb <= tb) to either child using a four-bit mask.
class PairInteractionSplitFinder {
public:
    splitx::Candidate best_pair_interaction(const splitx::SplitContext& ctx, const PairInteractionConfig& cfg,
                                            double axis_gain = -1.0) const {
        using namespace splitx;
        Candidate best;
        best.kind = SplitKind::PairInteraction;
        best.gain = NEG_INF;
        if (!ctx.Xb || !ctx.row_index || !ctx.row_g || !ctx.row_h || ctx.N < cfg.min_node_rows)
            return best;

        std::vector<int> features;
        ctx.for_each_active_feature([&](int f) {
            if (ctx.get_feature_bins(f) > 1)
                features.push_back(f);
        });
        if (features.size() < 2)
            return best;
        if (ctx.G) {
            const auto& histogram_gradients = *ctx.G;
            auto score = [&](int feature) {
                double value = 0.0;
                for (int bin = 0; bin < ctx.get_feature_bins(feature); ++bin) {
                    const size_t offset = ctx.get_histogram_offset(feature, bin);
                    if (offset < histogram_gradients.size())
                        value += std::abs(histogram_gradients[offset]);
                }
                return value;
            };
            std::ranges::sort(features, [&](int lhs, int rhs) { return score(lhs) > score(rhs); });
        }
        if (features.size() > static_cast<size_t>(std::max(2, cfg.max_features)))
            features.resize(static_cast<size_t>(std::max(2, cfg.max_features)));

        const int reduced_bins = std::clamp(cfg.interaction_bins, 2, 16);
        const double parent_gain = leaf_obj(ctx.Gp, ctx.Hp, ctx.hyp.lambda_, ctx.hyp.alpha_);
        struct PairScratch {
        ...
        const size_t cells = static_cast<size_t>(reduced_bins * reduced_bins);
        scratch.gradients.resize(cells);
        scratch.hessians.resize(cells);
        scratch.counts.resize(cells);
        auto& gradients = scratch.gradients;
        auto& hessians = scratch.hessians;
        auto& counts = scratch.counts;

        std::vector<int> interleaved_pairs;
        interleaved_pairs.reserve(features.size() * (features.size() - 1));
        for (size_t ia = 0; ia + 1 < features.size(); ++ia) {
            for (size_t ib = ia + 1; ib < features.size(); ++ib) {
                interleaved_pairs.push_back(features[ia]);
                interleaved_pairs.push_back(features[ib]);
            }
        }
        JointHistogramBatch device_batch;
        const int pair_count = static_cast<int>(interleaved_pairs.size() / 2);
        const bool has_device_batch = ctx.joint_histogram_builder &&
                                      ctx.joint_histogram_builder(ctx.joint_histogram_state, interleaved_pairs.data(),
                                                                  pair_count, reduced_bins, device_batch);
        const size_t device_stride = cells + 1;
        size_t pair_index = 0;

        for (size_t ia = 0; ia + 1 < features.size(); ++ia) {
            const int fa = features[ia];
            const int bins_a = ctx.get_feature_bins(fa);
            for (size_t ib = ia + 1; ib < features.size(); ++ib) {
                const int fb = features[ib];
                const int bins_b = ctx.get_feature_bins(fb);
                std::ranges::fill(gradients, 0.0);
                std::ranges::fill(hessians, 0.0);
                std::ranges::fill(counts, 0);
                double missing_g = 0.0, missing_h = 0.0;
                int missing_c = 0;

                if (has_device_batch) {
                    const size_t base = pair_index * device_stride;
                    for (size_t cell = 0; cell < cells; ++cell) {
                        gradients[cell] = device_batch.gradients[base + cell];
                        hessians[cell] = device_batch.hessians[base + cell];
                        counts[cell] = device_batch.counts[base + cell];
                    }
                    missing_g = device_batch.gradients[base + cells];
                    missing_h = device_batch.hessians[base + cells];
                    missing_c = device_batch.counts[base + cells];
                } else {
                    for (int local = 0; local < ctx.N; ++local) {
                        const int row = ctx.row_index[local];
                        const uint16_t ca = ctx.code_at(row, fa);
                        const uint16_t cb = ctx.code_at(row, fb);
                        if (ca == ctx.get_missing_bin_id(fa) || cb == ctx.get_missing_bin_id(fb)) {
                            missing_g += ctx.row_g[row];
                            missing_h += ctx.row_h[row];
                            ++missing_c;
                            continue;
                        }
                        const int ba = std::min(reduced_bins - 1, static_cast<int>(ca) * reduced_bins / bins_a);
                        const int bb = std::min(reduced_bins - 1, static_cast<int>(cb) * reduced_bins / bins_b);
                        const size_t offset = static_cast<size_t>(ba * reduced_bins + bb);
                        gradients[offset] += ctx.row_g[row];
                        hessians[offset] += ctx.row_h[row];
                        ++counts[offset];
                    }
                }

                for (int ta = 0; ta + 1 < reduced_bins; ++ta) {
                    for (int tb = 0; tb + 1 < reduced_bins; ++tb) {
                        double qg[4] = {}, qh[4] = {};
                        int qc[4] = {};
                        for (int ba = 0; ba < reduced_bins; ++ba) {
                            for (int bb = 0; bb < reduced_bins; ++bb) {
                                const int quadrant = (ba > ta ? 2 : 0) | (bb > tb ? 1 : 0);
                                const size_t offset = static_cast<size_t>(ba * reduced_bins + bb);
                                qg[quadrant] += gradients[offset];
                                qh[quadrant] += hessians[offset];
                                qc[quadrant] += counts[offset];
                            }
                        }
                        // Complement symmetry: bit zero is always assigned left.
                        for (uint8_t mask = 1; mask < 15; mask += 2) {
                            double base_g = 0.0, base_h = 0.0;
                            int base_c = 0;
                            for (int q = 0; q < 4; ++q) {
                                if (mask & (uint8_t{1} << q)) {
                                    base_g += qg[q];
                            ...
                            for (int direction = 0; direction < 2; ++direction) {
                                const bool missing_left = direction == 0;
                                const double gl = base_g + (missing_left ? missing_g : 0.0);
                                const double hl = base_h + (missing_left ? missing_h : 0.0);
                                const int cl = base_c + (missing_left ? missing_c : 0);
                                const double gr = ctx.Gp - gl;
                                const double hr = ctx.Hp - hl;
                                const int cr = ctx.Cp - cl;
                                if (!valid_children(hl, hr, cl, cr, ctx.hyp))
                                    continue;
                                const double gain = split_gain_from_parent(parent_gain, gl, hl, gr, hr, cl, cr, ctx.hyp,
                                                                           cfg.complexity_penalty, 1.0);
                                if (axis_gain > 0.0 && gain <= axis_gain * cfg.axis_guard_factor)
                                    continue;
                                if (gain <= best.gain)
                                    continue;
                                best.gain = gain;
                                best.pair_feature_a = fa;
                                best.pair_feature_b = fb;
                                best.pair_threshold_a = ((ta + 1) * bins_a - 1) / reduced_bins;
                                best.pair_threshold_b = ((tb + 1) * bins_b - 1) / reduced_bins;
                                best.pair_quadrant_mask = mask;
                                best.pair_missing_left = missing_left;
                            }
                        }
                    }
                }
...
public:
    ...
    splitx::Candidate best_oblique(const splitx::SplitContext& ctx, double /*axis_guard_gain*/ = -1.0) const {
        ...
        std::vector<int> ord;
        if (ctx.active_features)
            ord = *ctx.active_features;
        else {
            ord.resize(static_cast<size_t>(P));
            std::iota(ord.begin(), ord.end(), 0);
        }
        const int k = std::min(k_features, static_cast<int>(ord.size()));
        if (k < 2)
            return out;
        ...
        if (!cholesky_spd(A_chol, (int)S.size()))
            return out;
        w.resize(S.size());
        auto b_work = b;
    ...
    splitx::Candidate best_oblique_hist(const splitx::SplitContext& ctx) const {
        ...
        std::vector<double> A, b, w;
        build_normal_eq_from_codes(S, ctx.Xb, ctx.row_index, Nn, P, ctx, ctx.row_g, ctx.row_h, ridge + ctx.hyp.lambda_,
                                   A, b);
        std::vector<double> A_chol = A;
        if (!cholesky_spd(A_chol, (int)S.size()))
            return out;
        ...
        auto project_and_eval = [&](double& out_gain, double& out_thr, bool& out_mleft) {
            if (Nn <= std::max(2, hist_exact_cutover)) {
                ...
                build_projection_hist_from_codes(
                    S, w, ctx.Xb, ctx.row_index, Nn, P, ctx, ctx.row_g, ctx.row_h, Gz, Hz, Cz, Gm, Hm, Cm, zmin, zmax,
                    &z_edges, hist_use_quantile_binning && Nn <= std::max(2, hist_quantile_binning_max_rows),
                    hist_clip_quantile);
    ...
    splitx::Candidate best_oblique_interaction(const splitx::SplitContext& ctx, const InteractionSeededConfig& cfg,
        ...
        auto abs_corr_pair_ignore_nan = [&](int fa, int fb) -> double {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                if (!splitx::is_fin(xi) || !splitx::is_fin(yi))
                    continue;
                sx += xi;
                sy += yi;
                sxx += xi * xi;
        ...
        for (int pi = 0; pi < keep; ++pi) {
            ...
            double w0, w1;
            if (!splitx::solve_2x2(A00, A01, A11, b0, b1, w0, w1))
                continue;
