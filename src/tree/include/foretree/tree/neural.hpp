...
class NeuralLeafPredictor {
   public:
    explicit NeuralLeafPredictor(const NeuralLeafConfig& cfg)
        ...
        if (P_ <= 0)
            throw std::runtime_error(
                "NeuralLeafPredictor: input_dim must be positive");
        init_weights_();
    ...
    void fit(const double* X, int N, const double* y,
        ...
        for (int t = 0; t < M; ++t) {
            const int i = keep[t];
            std::copy_n(X + (size_t)i * (size_t)P_, (size_t)P_,
                        Xc.data() + (size_t)t * (size_t)P_);
            yc[(size_t)t] = y[i];
    ...
    void fit(const std::vector<std::vector<double>>& X,
        ...
        for (int i = 0; i < N; ++i) {
            if ((int)X[i].size() != P_) {
                std::cerr
                    << "NeuralLeafPredictor: dimension mismatch at sample " << i
                    << " got " << X[i].size() << " expected " << P_ << "\n";
                valid_ = false;
                return;
            }
            std::copy_n(X[i].data(), (size_t)P_,
                        X_flat.data() + (size_t)i * (size_t)P_);
        }
   ...
   private:
    ...
    bool shapes_ok_() const {
        return (P_ > 0) && (int)W1_.size() == H_ * P_ &&
               (int)b1_.size() == H_ && (int)W2_.size() == H_;
    }

    bool prepare_stats_(const double* X, int N, const double* y,
                        const double* weights, std::vector<int>& keep_idx,
                        std::vector<double>& keep_weights) {
        keep_idx.clear();
        keep_weights.clear();
        keep_idx.reserve(N);
        ...
        double wsum =
            std::accumulate(keep_weights.begin(), keep_weights.end(), 0.0);
    ...
    inline void normalize_row_(const double* x_raw, double* x_out) const {
        for (int j = 0; j < P_; ++j) {
            x_out[(size_t)j] =
                (x_raw[(size_t)j] - mu_[(size_t)j]) * inv_std_[(size_t)j];
        }
    }
    ...
    void train_impl_(const std::vector<double>& Xc,
        ...
        const int Mtr_unclamped =
            static_cast<int>(std::round(0.8 * static_cast<double>(M)));
        const int Mtr = std::min(std::max(1, Mtr_unclamped), M - 1);

        auto row_ptr = [&](int t) {
            return Xc.data() + (size_t)ord[t] * (size_t)P_;
        };
        ...
        for (int epoch = 0; epoch < cfg_.epochs; ++epoch) {
            const double progress =
                (cfg_.epochs > 1) ? double(epoch) / double(cfg_.epochs - 1)
                                  : 1.0;
            const double lr =
                cfg_.lr *
                (cfg_.cosine_lr
                     ? (0.5 + 0.5 * std::cos(3.141592653589793 * progress))
                     : 1.0);
            ...
            compute_gradients_(row_ptr, y_at, w_at, 0, Mtr, gW1, gW2, gb1, gb2,
                               wsum_tr);

            // Average and regularize
            const double inv_wsum = 1.0 / std::max(wsum_tr, 1e-12);
            for (size_t k = 0; k < gW1.size(); ++k)
                gW1[k] = gW1[k] * inv_wsum + cfg_.l2_reg * W1_[k];
            for (size_t k = 0; k < gW2.size(); ++k)
                gW2[k] = gW2[k] * inv_wsum + cfg_.l2_reg * W2_[k];
            ...
            adam_update_(W1_, mW1_, vW1_, gW1, lr, beta1, beta2, eps, bias1,
                         bias2);
            adam_update_(W2_, mW2_, vW2_, gW2, lr, beta1, beta2, eps, bias1,
                         bias2);
            adam_update_(b1_, mb1_, vb1_, gb1, lr, beta1, beta2, eps, bias1,
                         bias2);
            ...
            const double cur_val =
                compute_val_loss_(row_ptr, y_at, w_at, Mtr, M);
    ...
    double compute_baseline_loss_(YAt y_at, WAt w_at, int start,
                                  int end) const {
    ...
    double compute_val_loss_(RowPtr row_ptr, YAt y_at, WAt w_at, int start,
                             int end) const {
        double loss = 0.0, wsum = 0.0;
    ...
    template <typename RowPtr, typename YAt, typename WAt>
    void compute_gradients_(RowPtr row_ptr, YAt y_at, WAt w_at, int start,
                            int end, std::vector<double>& gW1,
                            std::vector<double>& gW2, std::vector<double>& gb1,
                            double& gb2, double& wsum_out) {
        std::vector<double> xnorm((size_t)P_);
        std::vector<double> hidden((size_t)H_);
        ...
        for (int t = start; t < end; ++t) {
            ...
            const double g_out =
                std::clamp(w * err, -cfg_.clip_elem, cfg_.clip_elem);
            gb2 += g_out;
            for (int h = 0; h < H_; ++h) {
            ...
            for (int h = 0; h < H_; ++h) {
                ...
                double g_h = std::clamp(g_out * W2_[(size_t)h], -cfg_.clip_elem,
                                        cfg_.clip_elem);
                gb1[(size_t)h] += g_h;
                const size_t off = (size_t)h * (size_t)P_;
                for (int j = 0; j < P_; ++j) {
    ...
    static void global_clip_(std::vector<double>& gW1, std::vector<double>& gW2,
                             std::vector<double>& gb1, double& gb2,
                             double max_norm) {
    ...
    static void adam_update_(std::vector<double>& w, std::vector<double>& m,
                             std::vector<double>& v,
                             const std::vector<double>& g, double lr, double b1,
                             double b2, double eps, double bias1,
                             double bias2) {
