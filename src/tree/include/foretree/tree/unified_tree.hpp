...
class UnifiedTree {
   public:
    ...
    void fit_with_row_ids(const QuantizedDataset& Xb,
   ...
   public:
    ...
    std::vector<double> predict(const std::vector<uint16_t>& Xb, int N, int P,
    ...
    inline PredictRawView resolve_predict_raw_view_(
        const double* Xraw_opt) const {
        if (Xraw_opt) {
    ...
    static void extend_path_(std::vector<PathElement>& path, int unique_depth,
        ...
        path[static_cast<size_t>(unique_depth)].pweight =
    ...
    static void unwind_path_(std::vector<PathElement>& path, int unique_depth,
                             int path_index) {
    ...
    static double unwound_path_sum_(const std::vector<PathElement>& path,
        ...
        for (int i = unique_depth - 1; i >= 0; --i) {
            if (one_fraction != 0.0) {
                const double tmp = next_one_portion *
                                   ...
                                   (static_cast<double>(i + 1) * one_fraction);
                total += tmp;
                next_one_portion = path[static_cast<size_t>(i)].pweight -
                                   tmp * zero_fraction *
                                       static_cast<double>(unique_depth - i) /
                                       static_cast<double>(unique_depth + 1);
            } else {
    ...
    void tree_shap_recursive_(int node_id, const uint16_t* row_binned,
                              int row_idx, const PredictRawView& raw_view,
                              double* phi, int unique_depth,
        ...
        if (packed_tree_.leaf_flags[static_cast<size_t>(node_id)] != 0) {
            const double leaf_value =
                packed_tree_.leaf_values[static_cast<size_t>(node_id) * K_];
        ...
        tree_shap_recursive_(hot_index, row_binned, row_idx, raw_view, phi,
                             unique_depth + 1, path,
                             hot_zero_fraction * incoming_zero_fraction,
                             incoming_one_fraction, split_feature);
        tree_shap_recursive_(cold_index, row_binned, row_idx, raw_view, phi,
                             unique_depth + 1, path,
   ...
   public:
    ...
    std::vector<double>
        feat_cover_;  // sum of hessian (weight) at nodes using this feature
    ...
    std::pair<double, double> compute_goss_rates_(const Node& n) const {
        ...
        if (cfg_.goss.adaptive) {
            ...
            a = std::min(
                0.5, cfg_.goss.top_rate *
                         (1.0 + cfg_.goss.adaptive_scale * variance_factor));
            b = std::min(
                0.5, cfg_.goss.other_rate *
                         (1.0 + cfg_.goss.adaptive_scale * variance_factor));
        }
    ...
    std::unordered_map<int, double> build_goss_weight_map_(
        const Node& n) const {
    ...
    SplitEngineConfig make_split_engine_config_() const {
        ...
        ecfg.categorical_max_selected_categories =
            cfg_.categorical_max_selected_categories;
    ...
    std::pair<std::vector<double>, std::vector<double>> sum_grad_hess_for_rows_(
    ...
    void accum_goss_weighted_(Node& n, bool use_goss) {
        ...
        for (size_t c = 0; c < Gt.size(); ++c) {
            ...
            n.goss_weighted_H[c] = cfg_.goss.scale_hessian
                                       ? (Ht[c] + n.goss_rest_scale * Hr[c])
                                       : (Ht[c] + Hr[c]);
        }
    }
    ...
    // Multiclass: compute K leaf values (one per class). For K=1, same as
    // leaf_value_scalar_.
    ...
    node_totals_for_leaf_(const Node& n) const {
        const int K_ = std::max(n.K, 1);
        std::vector<double> G_out(static_cast<size_t>(K_), 0.0);
        std::vector<double> H_out(static_cast<size_t>(K_), 0.0);
        for (int c = 0; c < K_; ++c) {
            G_out[static_cast<size_t>(c)] =
                node_uses_goss_(n) ? n.goss_weighted_G[static_cast<size_t>(c)]
                                   : n.G[static_cast<size_t>(c)];
            H_out[static_cast<size_t>(c)] =
                node_uses_goss_(n) ? n.goss_weighted_H[static_cast<size_t>(c)]
    ...
    double priority_(double gain, const Node& nd) const {
        ...
        if (cfg_.leaf_depth_penalty > 0.0)
            pr /=
                (1.0 + cfg_.leaf_depth_penalty * static_cast<double>(nd.depth));
    ...
    class HistogramBuilder {
       ...
       public:
        ...
        bool build_cuda(const std::vector<int>& rows,
            ...
            const auto device_histogram =
                T.cuda_histogram_engine_->build_histogram(device_rows);
            for (int feature : feats) {
                const size_t begin =
                    T.feature_offsets_[static_cast<size_t>(feature)];
                const size_t count = static_cast<size_t>(
                    T.missing_ids_per_feat_[static_cast<size_t>(feature)] + 1);
                for (size_t index = begin; index < begin + count; ++index) {
                    hist.G[index] =
                        static_cast<double>(device_histogram.gradients[index]);
                    hist.H[index] =
                        static_cast<double>(device_histogram.hessians[index]);
                    hist.C[index] =
                        static_cast<int>(device_histogram.counts[index]);
                }
            }
            return true;
        }
        ...
        void build_for_rows(const std::vector<int>& rows,
                            const std::vector<int>& feats,
                            HistPair& hist) const {
#ifdef FORETREE_HAS_CUDA
            if (build_cuda(rows, feats, hist)) return;
#endif
            dispatch_cpu_(
                static_cast<int>(rows.size()),
                [&](int position) { return rows[position]; }, feats, hist);
        }

        void build_for_range(int lo, int hi, const std::vector<int>& feats,
                             HistPair& hist) const {
#ifdef FORETREE_HAS_CUDA
            ...
            dispatch_cpu_(
                hi - lo,
                [&](int position) {
                    return index_pool[static_cast<size_t>(lo + position)];
                },
                feats, hist);
        }
    };
    ...
    struct HistogramProvider {
        ...
        std::shared_ptr<HistPair> build_histogram(
            const Node& nd, const std::vector<int>& feats) const {
            const bool goss_here = T.node_uses_goss_(nd);

            if (nd.hist_valid && nd.histogram && nd.hist_features == feats &&
                nd.hist_goss_weighted == goss_here)
                return nd.histogram;

            std::shared_ptr<HistPair> hist;
            const bool is_full_root =
                nd.lo == 0 && nd.hi == static_cast<int>(T.index_pool_.size());
            if (!goss_here && is_full_root && T.tree_histogram_ &&
                feats == T.tree_features_ && !T.tree_subsample_applied_) {
                hist = T.tree_histogram_;
            } else {
                hist = T.acquire_histogram_();
            }

            if (!is_full_root && !goss_here && T.tree_histogram_ &&
                feats == T.tree_features_ && nd.C >= T.cfg_.cache_threshold &&
       ...
       private:
        void derive_from_tree_histogram_(const Node& nd,
            ...
            // Build directly from node rows when the node is smaller than its
            // complement.
        ...
        void build_from_rows_(const Node& nd, const std::vector<int>& feats,
            ...
            if (use_goss) {
                ...
                HistogramBuilder(T, index_pool)
                    .build_for_rows(work_rows, feats, hist);
            } else {
                HistogramBuilder(T, index_pool)
                    .build_for_range(nd.lo, nd.hi, feats, hist);
            }
        }
        ...
        void build_with_goss_(const Node& nd, const std::vector<int>& feats,
            ...
            if (total < T.cfg_.goss.min_node_size || !nd.goss_samples_valid_ ||
                nd.goss_top_indices_.empty()) {
                HistogramBuilder(T, index_pool)
                    .build_for_range(nd.lo, nd.hi, feats, hist);
                return;
            }

            auto top_hist = T.hist_pool_ ? T.hist_pool_->get()
                                         : std::make_unique<HistPair>();
            auto rest_hist = T.hist_pool_ ? T.hist_pool_->get()
                                          : std::make_unique<HistPair>();
            ...
            if (!nd.goss_rest_indices_.empty())
                builder.build_for_rows(nd.goss_rest_indices_, feats,
                                       *rest_hist);

            for (size_t i = 0; i < hist.G.size(); ++i) {
                hist.G[i] =
                    top_hist->G[i] + nd.goss_rest_scale * rest_hist->G[i];
                hist.H[i] = T.cfg_.goss.scale_hessian
                                ? (top_hist->H[i] +
                                   nd.goss_rest_scale * rest_hist->H[i])
       ...
       public:
        foretree::splitx::Candidate best_split(
            const Node& nd, const SplitHyper& hyp,
            const std::vector<int8_t>* mono) {
            auto feats = nd.hist_features.empty() ? T.select_features_()
                                                  : nd.hist_features;
            ...
            for (int c = 0; c < K_; ++c) {
                Gtot_vec[static_cast<size_t>(c)] =
                    nd.uses_goss ? nd.goss_weighted_G[static_cast<size_t>(c)]
                                 : nd.G[static_cast<size_t>(c)];
                Htot_vec[static_cast<size_t>(c)] =
                    nd.uses_goss ? nd.goss_weighted_H[static_cast<size_t>(c)]
                                 : nd.H[static_cast<size_t>(c)];
            }
            ...
            if (joint_cuda_state.engine && !nd.uses_goss) {
                ...
                ctx.joint_histogram_builder =
                    [](void* opaque, const int* interleaved_pairs,
                       int pair_count, int reduced_bins,
                       splitx::JointHistogramBatch& output) -> bool {
                    ...
                    for (int pair = 0; pair < pair_count; ++pair) {
                        pairs[static_cast<size_t>(pair)] = cuda::FeaturePair{
                            static_cast<uint32_t>(interleaved_pairs[2 * pair]),
                            static_cast<uint32_t>(
                                interleaved_pairs[2 * pair + 1])};
                    }
                    rows.resize(static_cast<size_t>(state.row_count));
                    std::transform(
                        state.rows, state.rows + state.row_count, rows.begin(),
                        [](int row) { return static_cast<uint32_t>(row); });
                    const auto result = state.engine->build_joint_histograms(
                        pairs, reduced_bins, rows);
                    output.reduced_bins = result.reduced_bins;
                    output.gradients.assign(result.gradients.begin(),
                                            result.gradients.end());
                    output.hessians.assign(result.hessians.begin(),
                                           result.hessians.end());
                    output.counts.assign(result.counts.begin(),
                                         result.counts.end());
            ...
            if (nd.uses_goss) ecfg.enable_pair_interactions = false;

            auto result =
                Splitter::best_split(ctx, SplitEngine::Histogram, ecfg);
            return result;
        }
    };
    ...
    struct ExactProvider {
        ...
        foretree::splitx::Candidate best_split(
            ...
            bool any_missing =
                std::ranges::any_of(Cmiss, [](int c) { return c > 0; });
            ...
            if (!needs_local_oblique_data) {
                return Splitter::best_split(ctx, SplitEngine::Exact, ecfg,
                                            T.Xraw_, T.P_, rows.data(),
                                            static_cast<int>(rows.size()),
            ...
            if (n_local <= 1) {
                return Splitter::best_split(ctx, SplitEngine::Exact, ecfg,
                                            T.Xraw_, T.P_, rows.data(), n_local,
                                            hyp.missing_policy, T.Xmiss_);
            }
            ...
            const size_t local_cells =
                static_cast<size_t>(n_local) * static_cast<size_t>(T.P_);
            ...
            for (int i = 0; i < n_local; ++i) {
                ...
                const size_t row_local =
                    static_cast<size_t>(i) * static_cast<size_t>(T.P_);
                const size_t row_global =
                    static_cast<size_t>(r) * static_cast<size_t>(T.P_);
                for (int f = 0; f < T.P_; ++f) {
                    const size_t idx_local = row_local + static_cast<size_t>(f);
                    const size_t idx_global =
                        row_global + static_cast<size_t>(f);
                    const double x = T.Xraw_[idx_global];
                    local_X[idx_local] = x;
                    col_storage[static_cast<size_t>(f) *
            ...
            for (int f = 0; f < T.P_; ++f) {
                xcols[static_cast<size_t>(f)] =
                    col_storage.data() +
                    static_cast<size_t>(f) * static_cast<size_t>(n_local);
            }
            ...
            // Recompute missing aggregates in local indexing when we switch to
            // local row_g/row_h.
            ...
            for (int i = 0; i < n_local; ++i) {
                ...
                for (int f = 0; f < T.P_; ++f) {
                    ...
                    const bool miss = T.Xmiss_ ? (local_miss[idx] != 0)
                                               : !std::isfinite(local_X[idx]);
                    if (miss) {
                        local_Gmiss[static_cast<size_t>(f)] +=
                            local_g[static_cast<size_t>(i)];
                        local_Hmiss[static_cast<size_t>(f)] +=
                            local_h[static_cast<size_t>(i)];
                        local_Cmiss[static_cast<size_t>(f)] += 1;
                    }
                }
            }
            const bool any_local_missing =
                std::ranges::any_of(local_Cmiss, [](int c) { return c > 0; });
            ...
            return Splitter::best_split(ctx, SplitEngine::Exact, ecfg,
                                        local_X.data(), T.P_, local_idx.data(),
                                        n_local, hyp.missing_policy,
