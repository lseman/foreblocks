...
inline bool categorical_from_unique_count(int unique_count,
                                          const HistogramConfig& cfg) {
    return unique_count > 0 && unique_count <= categorical_unique_limit(cfg);
}

inline std::vector<double> resample_edges_linear(
    const std::vector<double>& edges, int new_nb) {
    ...
    for (int k = 0; k <= new_nb; ++k) {
        ...
        out[static_cast<size_t>(k)] =
            (1.0 - t) * edges[static_cast<size_t>(i)] +
            t * edges[static_cast<size_t>(i + 1)];
    }
    for (size_t i = 1; i < out.size(); ++i) {
        if (!(out[i] > out[i - 1])) {
            out[i] = std::nextafter(out[i - 1],
...
inline void finalize_feature_bins(FeatureBins& fb, const HistogramConfig& cfg,
    ...
    if (fb.stats.suggested_bins <= 0) {
        ...
        fb.stats.suggested_bins =
            std::clamp(fb.stats.suggested_bins, suggested_min, max_bins);
    }
    _check_uniform(fb);
}
...
inline bool apply_common_categorical_precheck(
    ...
    fb.stats.suggested_bins =
        std::min(unique_count, std::max(1, max_bins_for_feature));
...
struct UniformBinner final : IBinningStrategy {
    FeatureBins create_bins(const std::vector<double>& values,
        ...
        if (apply_common_categorical_precheck(fb, finite_values, cfg,
    ...
    FeatureBins create_bins(const std::vector<double>& values,
        ...
        auto update_step = [&](std::vector<double>& c) {
            ...
            for (int k = 1; k < K; ++k) {
                if (!(c[k] > c[k - 1])) {
                    c[k] = std::nextafter(
                        c[k - 1], std::numeric_limits<double>::infinity());
                }
            }
        };
        ...
        for (size_t i = 1; i < fb.edges.size(); ++i) {
            if (!(fb.edges[i] > fb.edges[i - 1])) {
                fb.edges[i] = std::nextafter(
                    fb.edges[i - 1], std::numeric_limits<double>::infinity());
            }
        }
...
struct GradientAwareBinner final : IBinningStrategy {
    FeatureBins create_bins(const std::vector<double>& values,
        ...
        const int min_bins_for_feature =
            std::clamp(cfg.min_bins, 1, max_bins_for_feature);
        const int nb = std::clamp(
            std::max(16, static_cast<int>(std::lround(
                             static_cast<double>(cfg.coarse_bins) * comp))),
...
struct TwoStageBinner final : IBinningStrategy {
    FeatureBins create_bins(const std::vector<double>& values,
        ...
        fb.stats.unique_count = static_cast<int>(uniq.size());
        if (static_cast<int>(uniq.size()) <=
            std::min(max_bins_for_feature, 32)) {
            fb.edges = _midpoint_edges_of_unique(uniq);
            fb.stats.suggested_bins = fb.n_bins();
            fb.stats.allocation_reason = "small_unique_midpoints";
            finalize_feature_bins(fb, cfg, max_bins_for_feature, true);
        ...
        const int target_default = std::max(
            min_bins_for_feature,
            std::min(max_bins_for_feature,
                     static_cast<int>(std::lround(
                         static_cast<double>(cfg.target_bins) * comp))));
        const int target_bins = std::clamp(target_default, min_bins_for_feature,
                                           max_bins_for_feature);
        const int coarse_bins = std::clamp(coarse_default, 2, target_bins);

        std::vector<double> coarse_w(v.size());
        for (size_t i = 0; i < v.size(); ++i)
            coarse_w[i] = std::max(cfg.eps, h[i]);
        std::vector<double> coarse_edges =
            weighted_quantile_edges(v, coarse_w, coarse_bins);

        const int coarse_nb =
            std::max(1, static_cast<int>(coarse_edges.size()) - 1);
        ...
        std::vector<std::vector<double>> bin_values(
            static_cast<size_t>(coarse_nb));
        std::vector<std::vector<double>> bin_weights(
            static_cast<size_t>(coarse_nb));
        std::vector<double> bin_scores(static_cast<size_t>(coarse_nb), 0.0);
        std::vector<double> bin_hsum(static_cast<size_t>(coarse_nb), 0.0);

        for (size_t i = 0; i < v.size(); ++i) {
            const auto it = std::upper_bound(coarse_edges.begin() + 1,
                                             coarse_edges.end() - 1, v[i]);
            ...
            bin_weights[static_cast<size_t>(b)].push_back(
                std::max(cfg.eps, wi));
        ...
        for (int b = 0; b < coarse_nb; ++b) {
            if (bin_hsum[static_cast<size_t>(b)] > 0.0) {
                const double normalized =
                    ...
                    (bin_hsum[static_cast<size_t>(b)] + cfg.eps);
                bin_scores[static_cast<size_t>(b)] =
                    normalized *
                    std::sqrt(bin_hsum[static_cast<size_t>(b)] + cfg.eps);
            } else {
        ...
        if (extra > 0) {
            if (total_score <= 0.0) {
                for (int t = 0; t < extra; ++t)
                    add[static_cast<size_t>(t % coarse_nb)]++;
            } else {
                ...
                for (int b = 0; b < coarse_nb; ++b) {
                    ...
                    rem[static_cast<size_t>(b)] =
                        exact - static_cast<double>(whole);
                    assigned += whole;
                }
        ...
        for (int b = 0; b < coarse_nb; ++b) {
            ...
            if (add_b > 0 && !bin_values[static_cast<size_t>(b)].empty()) {
                std::vector<double> local = weighted_quantile_edges(
                    bin_values[static_cast<size_t>(b)],
                    bin_weights[static_cast<size_t>(b)], add_b + 1);
            ...
            if (!(upper > fb.edges.back())) {
                upper = std::nextafter(fb.edges.back(),
                                       std::numeric_limits<double>::infinity());
            }
...
struct AdaptiveBinner final : IBinningStrategy {
    FeatureBins create_bins(const std::vector<double>& values,
        ...
        const int min_bins_for_feature =
            std::clamp(cfg.min_bins, 1, max_bins_for_feature);
        const int target_bins =
            std::clamp(fb.stats.suggested_bins, min_bins_for_feature,
                       max_bins_for_feature);
        ...
        if (fb.stats.is_categorical) {
            ...
            fb.stats.unique_count = static_cast<int>(u.size());
            fb.stats.suggested_bins =
                std::min(static_cast<int>(u.size()), max_bins_for_feature);
            fb.edges = _midpoint_edges_of_unique(u);
            finalize_feature_bins(fb, cfg, max_bins_for_feature, false);
            return fb;
        }
