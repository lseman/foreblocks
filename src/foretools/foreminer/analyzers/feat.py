...
class FeatureEngineeringAnalyzer(AnalysisStrategy):
    ...
    def _suggest_numeric_transforms(
        ...
        y_series = (
            pd.to_numeric(data[target_name], errors="coerce")
            if target_name in (data.columns if target_name else [])
            else None
        )
        y_present = (
            y_series is not None
            and y_series.notna().sum() >= 5
            and y_series.nunique() > 1
        )
        hsic_weight = float(
            getattr(cfg, "transform_hsic_weight", 0.25)
        )  # contributes to ranking if y_present
        ...
        def _catalog_for(col: str, a: np.ndarray) -> list[tuple[str, callable, str]]:
            ...
            qt_u = QuantileTransformer(
                output_distribution="uniform",
                n_quantiles=min(100, max(10, n)),
                random_state=rs,
                subsample=int(1e9),
            )
            qt_n = QuantileTransformer(
            ...
            out.append(
                (
                    "q_uniform",
                    lambda v: qt_u.transform(v.reshape(-1, 1)).ravel(),
            ...
            out.append(
                (
                    "q_normal",
                    lambda v: qt_n.transform(v.reshape(-1, 1)).ravel(),
            ...
            out.append(
                (
                    "yeo_johnson",
                    lambda v: pt_yj.transform(v.reshape(-1, 1)).ravel(),
            ...
            if can_boxcox:
                ...
                try:
                    ...
                    out.append(
                        (
                            "box_cox",
                            lambda v: pt_bc.transform(v.reshape(-1, 1)).ravel(),
            ...
            out.append(
                (
                    "clip_iqr3",
                    lambda v: np.clip(v, lo_iqr3, hi_iqr3),
            ...
            out.append(
                (
                    ...
                    lambda v: pd.qcut(
                        pd.Series(v),
                        q=min(qbins, max(2, len(np.unique(v)))),
                        labels=False,
                        duplicates="drop",
                    ).to_numpy(),
        ...
        def _score_transform(
            vec: np.ndarray, yvec: np.ndarray | None
            ...
            ku = (
                float(sps.kurtosis(v, fisher=False, bias=False))
                if v.size > 3
                else np.nan
            )
            score = 0.0
            ...
            if np.isfinite(jb):
                score += np.clip(jb, 0.0, 1.0) * 1.0
            if np.isfinite(sk):
                score += (1.0 / (1.0 + abs(sk))) * 0.5
            if np.isfinite(ku):
                score += (1.0 / (1.0 + abs(ku - 3.0))) * 0.5
            ...
            return score, {
                ...
                "skew": sk,
                "kurt": ku,
                "hsic": (hs if y_present else np.nan),
            }
        ...
        for col in numeric_cols:
            ...
            q1, q3 = np.percentile(a, [25, 75])
            iqr = float(q3 - q1)
            skew0 = float(sps.skew(a, bias=False))
            kurt0 = float(sps.kurtosis(a, fisher=False, bias=False))
            jb0 = _jb_p(_sample(a))
            sw0 = _sw_p(_sample(a))
            ...
            if y_present:
                ...
                y_aligned = y_arr_full[
                    s.index
                ]  # safe because s comes from data[col].dropna()
            ...
            for name_key, fn, as_str in cats:
                try:
                    ...
                    if (abs(skew0) > 2.0 or kurt0 > 6.0) and any(
                        k in name_key
                        for k in ("yeo", "box", "log", "asinh", "q_normal")
                    ):
                        sc += 0.05
            ...
            details[col] = {
                ...
                "scored": [
                    {"expr": s, "score": float(sc), **meta}
                    for (s, sc, meta) in scored[:max_k]
                ],
                "recommended": rec,
            }
    ...
    def _rank_features(
        ...
        if y_present:
            ...
            hsic_estimator = getattr(
                cfg, "rank_hsic_estimator", "biased"
            )  # or "unbiased"
            ...
            scorer = HSIC(
                kernel_x=hsic_kernel_x,
                kernel_y=hsic_kernel_y,
                estimator=hsic_estimator,
                normalize=hsic_normalize,
                use_numba=use_numba,
                random_state=rs,
            )
            for c in X.columns:
        ...
        if (
            getattr(cfg, "enable_shap", False)
            and y_present
            and len(X) < getattr(cfg, "shap_max_n", 12000)
        ):
            try:
                import shap
    ...
    def _suggest_interactions(
        ...
        hsic_estimator = getattr(
            cfg, "hsic_estimator", "biased"
        )  # "biased" | "unbiased" | "block" | "rff"...
        ...
        with np.errstate(all="ignore"):
            corr_matrix = np.abs(
                work.corr(method="pearson", min_periods=min_periods).values
            )
        ...
        # ---------- HSIC scorers ----------
        hsic_pair = HSIC(
            kernel_x=hsic_kernel_x_pairs,
            kernel_y=hsic_kernel_y_pairs,
            estimator=hsic_estimator,
            normalize=hsic_normalize,
            use_numba=hsic_use_numba,
        ...
        hsic_tgt = HSIC(
            kernel_x=hsic_kernel_x_tgt,
            kernel_y=hsic_kernel_y_tgt,
            estimator=hsic_estimator,
            normalize=hsic_normalize,
            use_numba=hsic_use_numba,
            random_state=getattr(cfg, "random_state", 42),
        ...
        def _build_Z(mask_base: np.ndarray, exclude: tuple) -> tuple:
            ...
            m = (
                mask_base if mask_base is not None else np.ones(len(Z_full), bool)
            ) & mZ
        ...
        for a, b in combinations(cols, 2):
            ...
            for expr in exprs:
                expr_candidates[expr] = (
                    ...
                    b,
                    mask,
                )  # store names+mask to build values later
                    ...
                    if cond_on:
                        ...
                        if Zm is not None:
                            ...
                            if xf.size >= max(hsic_min_samples, cond_min_n):
                                try:
                                    s = hsic_tgt.conditional_score(
                                        xf, yf, Zm, lam=cond_lam
                                    )
                                except Exception:
        ...
        if (
            ...
            and len(final_expressions) < max_pairs
        ):
            ...
            for a, b, _ in pair_scores[:hl_seed_pairs_top]:
                for c in cols:
                    ...
                    candidates = {
                        ...
                        f"np.median([{a},{b},{c}], axis=0)": np.median(
                            np.vstack([x1, x2, x3]), axis=0
                        ),
                    }
    ...
    def _suggest_encodings(
        ...
        task: str = getattr(
            cfg, "task", "auto"
        )  # "regression" | "binary" | "multiclass" | "auto"
        time_col: str | None = getattr(
            cfg, "time_col", None
        )  # for leakage-safe encoders
        group_col: str | None = getattr(
            cfg, "group_col", None
        )  # group-aware CV, leakage guard
        rs = getattr(cfg, "random_state", 42)
            ...
            if task == "auto":
                if pd.api.types.is_numeric_dtype(y):
                    ...
                    if nun == 2:
                        ...
                        float(v).is_integer() for v in y.dropna().unique()
                    ):
                        y_kind = "multiclass"
                    else:
                        y_kind = "regression"
        ...
        for col in categorical_cols:
            ...
            if null_pct > 0:
                notes.append(
                    f"Missing {null_pct:.1f}% → treat NaN as its own level + MissingIndicator."
                )
            notes.append(
                "Set handle_unknown='infrequent_if_exist' / 'ignore' to avoid errors."
            )
            if group_col:
                notes.append(
                    "Use GroupKFold/LeaveOneGroupOut for supervised encoders (group leakage)."
                )
            if time_col:
                notes.append(
                    "Use time-based CV (purged CV) for supervised encoders (temporal leakage)."
                )
            ...
            if bucket == "very_low":
                ...
                enc += [
                    "FrequencyEncoder",
                    "HashingEncoder(n_features≈min(2*k, 512))",
                    "BinaryEncoder",
                ]
            else:  # extreme
                enc += [
                    "HashingEncoder(n_features≈max(1024, 2*k))",
                    "FrequencyEncoder",
                    "LeaveOneOutEncoder (CV)",
                ]
            ...
            if y_kind is not None:
                if y_kind == "regression":
                    ...
                    if bucket in {"low", "medium"}:
                        ...
                        enc = [
                            "CatBoostEncoder (KFold CV, ordered)",
                            "TargetEncoder (KFold CV, smoothing)",
                        ] + enc
                elif y_kind == "binary":
                    # WOE is principled for binary classification
                    if nunique <= 50:
                        enc = [
                            "WOEEncoder (KFold CV, regularized)",
                            "TargetEncoder (KFold CV, smoothing)",
                        ] + enc
                    else:
                        enc = [
                    ...
                    if bucket in {"low", "medium"}:
                        enc = [
                            "TargetEncoder (KFold CV, per-class)",
                            "OneHot(drop_first=False)",
                        ] + enc
                    else:
            ...
            if nunique > 0.9 * len(s):
                notes.append(
                    "Looks like an ID-like field → avoid supervised encoders; prefer hashing/frequency."
                )
            ...
            for e in enc:
                ...
                if (
                    "TargetEncoder" in e
                    or "CatBoostEncoder" in e
                    or "WOEEncoder" in e
                    or "LeaveOneOut" in e
                ):
                    score += 2 if y_kind else -1
                # Prefer hashing/frequency for high/extreme
                if bucket in {"high", "extreme"} and (
                    "Hashing" in e or "Frequency" in e
                ):
                    score += 2
            ...
            details[col] = {
                ...
                "leakage_advice": (
                    ...
                    if y_kind
                    else "Unsupervised encoders only (no target)."
                ),
                "params_hints": {
                    "TargetEncoder": {
                        "cv": "KFold/GroupKFold/TimeSeriesSplit",
                        "smoothing": "1–50",
                        "noise": "0.01–0.1",
                    },
                    "CatBoostEncoder": {
                    ...
                    "WOEEncoder": {
                        "binary_only": True,
                        "min_samples": "≥50 per bin",
                        "regularization": "strong",
                    },
                    "HashingEncoder": {
                        "n_features": "choose to keep collision rate <5%"
                    },
                    "RareCategoryEncoder": {
                        "threshold": "0.5%–2%",
                        "merge": "to '__rare__'",
