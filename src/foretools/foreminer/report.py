...
class DatasetReportPrinter:
    ...
    def print_insights(self, thresholds=None):
        """Generate a comprehensive, well-formatted dataset analysis report."""
        section, metric, rec, pct, safe_get = (
            report_section,
            report_metric,
            report_recommendation,
            report_pct,
        ...
        self.print_executive_summary(TH)
        self.warmup_core_analyses()
        self.print_distribution_analysis(TH)
        self.print_correlation_analysis(TH)
        self.print_sota_insights()
        ...
        if patt:
            ...
            if ft:
                ...
                if ft.get("transformable_to_normal"):
                    ...
                    print(
                        f"🔄 Transformable: {top_list(ft['transformable_to_normal'])}"
                    )
                ...
                pattern_specs = {
                    "nonlinear": {
                        ...
                        "extras": lambda r: (
                            ...
                            if r.get("functional_form") != "none"
                            and r.get("functional_r2", 0) > 0.3
                            else None
                        ),
                    ...
                    "complex": {
                        ...
                        "extras": lambda r: [
                            (
                                f"Ensemble strength: {r['ensemble_score']:.3f}"
                                if r.get("ensemble_score")
                                else None
                            ),
                            (
                                f"Copula dependence: {r['copula_tau']:.3f}"
                                if r.get("copula_tau", 0) > 0.2
                                else None
                            ),
                        ],
                    ...
                    "ensemble_strong": {
                        ...
                        "extras": lambda r: [
                            ...
                            (
                                f"Distance: {r['distance_corr']:.3f}"
                                if r.get("distance_corr", 0) > 0
                                else None
                            ),
                            (
                                f"MI: {r['mutual_info']:.3f}"
                                if r.get("mutual_info", 0) > 0
                                else None
                            ),
                    ...
                    "tail_dependence": {
                        ...
                        "extras": lambda r: [
                            ...
                            (
                                f"Lower tail: {r.get('lower_tail_dep', 0):.3f}"
                                if r.get("lower_tail_dep", 0) > 0
                                else None
                            ),
                            (
                                f"Asymmetric: +{r['asymmetric_pos']:.3f}, -{r['asymmetric_neg']:.3f}"
                                if max(
                                    r.get("asymmetric_pos", 0),
                                    r.get("asymmetric_neg", 0),
                                )
                                > 0.02
                                else None
                            ),
                    ...
                    "copula_dependence": {
                        ...
                        "extras": lambda r: [
                            ...
                            (
                                f"Tail coefficient: {r.get('tail_dep', 0):.3f}"
                                if r.get("tail_dep", 0) > 0
                                else None
                            ),
                        ],
                    ...
                    for r in rel[ptype][:3]:
                        ...
                        for field, label, prec in spec["fields"]:
                            if field in r:
                                ...
                                fmt_val = (
                                    f"{val:.{prec}f}"
                                    if isinstance(val, (int, float)) and prec
                                    else str(val)
                                )
            ...
            if fits:
                ...
                for feat, info in list(fits.items())[:5]:
                    ...
                    metric(
                        "AIC", f"{aic:.2f}" if not np.isnan(aic) else "N/A", indent=1
                    )
            ...
            if total_patterns > 0:
                ...
                if any(
                    r.get("ensemble_score", 0) > 0.7
                    for patterns in rel.values()
                    for r in patterns
                ):
                    print(
        ...
        if not out or "error" in out:
            ...
            if summary:
                ...
                status = (
                    "warning" if rate > 0.10 else "good" if rate > 0.05 else "excellent"
                )
                metric("Overall Outlier Rate", f"{rate:.1%}", status=status)
                if summary.get("best_method"):
                    metric("Best Method", summary["best_method"].upper())
            ...
            if prep:
                ...
                metric(
                    "Scaling Method",
                    prep.get("scaling_method", "unknown").replace("_", " ").title(),
                )
                metric(
                    "Missing Data Strategy",
                    prep.get("handling_strategy", "unknown").replace("_", " ").title(),
                )
            ...
            if methods:
                ...
                for mname, res in methods.items():
                    ...
                    if "isolation_score" in ev:
                        metric(
                            "Isolation Score", f"{ev['isolation_score']:.3f}", indent=1
                        )
            ...
            if recs:
                ...
                for i, r in enumerate(recs[:5], 1):
                    clean = (
                        r.replace("🏆", "").replace("⚠️", "").replace("✅", "").strip()
                    )
                    pr = (
                        "high"
                        if "best method" in clean.lower()
                        else (
                            "medium"
                            if "high" in clean.lower()
                            and ("rate" in clean.lower() or "outlier" in clean.lower())
                            else "normal"
                        )
                    )
                    rec(f"{i}. {clean}", pr)
        ...
        if not cl or "error" in cl:
            ...
            if ok:
                ...
                metric(
                    "Confidence Level", conf.title() if isinstance(conf, str) else conf
                )
        ...
        if ts:
            ...
            if not stationarity_df.empty:
                ...
                metric(
                    "Stationary Series",
                    f"{stationary}/{total} ({pct(stationary, total)})",
                )
                metric("Forecast-Ready", f"{ready}/{total} ({pct(ready, total)})")

                # ---------- Stationarity ----------
                section("Stationarity Assessment", 3)
                st_feats = stationarity_df.loc[
                    stationarity_df["is_stationary"], "feature"
                ].tolist()
                ...
                if not non_st.empty:
                    ...
                    for _, r in non_st.head(3).iterrows():
                        metric(
                            r["feature"],
                            str(r.get("stationarity_type", "unknown")),
                            indent=1,
                        )
                ...
                for k, v in trends.items():
                    if v.get("linear_significant") and v.get("trend_direction") not in {
                        "no_trend",
                        "stable",
                    }:
                ...
                for k, v in seas.items():
                    ...
                    if cls in {"moderate", "strong"}:
                        seas_list.append(
                            (
                                k.replace("_seasonality", ""),
                                v.get("stl_seasonal_strength", 0) or 0,
                            )
                        )
                ...
                candidate_feats = set(lag_sugg.keys()) | {
                    k.replace("_seasonality", "") for k in seas.keys()
                }
                for f in sorted(candidate_feats):
                    ...
                    if (
                        rec_lags
                        or seas_lags
                        or stl_period is not None
                        or seas_strength is not None
                    ):
                        lag_rows.append(
                            {
                                "feature": f,
                                ...
                                "stl_period": (
                                    int(stl_period)
                                    if isinstance(stl_period, (int, float, np.integer))
                                    and not pd.isna(stl_period)
                                    else None
                                ),
                                "seas_strength": (
                                    float(seas_strength)
                                    if isinstance(seas_strength, (int, float))
                                    and not pd.isna(seas_strength)
                                    else None
                                ),
                            }
                        )
                ...
                if lag_rows:
                    ...
                    for r in lag_rows_sorted[:N]:
                        ...
                        metric(
                            r["feature"], " | ".join(parts) if parts else "—", indent=1
                        )
                    ...
                    all_rec_lags = [
                        lag for r in lag_rows for lag in (r["rec_lags"] or [])
                    ]
                    all_seas_lags = [
                        lag for r in lag_rows for lag in (r["seas_lags"] or [])
                    ]
                    ...
                    if rec_counts:
                        most_rec = ", ".join(
                            f"{k}({v})" for k, v in rec_counts.most_common(3)
                        )
                        summary_bits.append(f"Top lags: {most_rec}")
                    if seaslag_counts:
                        most_seas = ", ".join(
                            f"{k}({v})" for k, v in seaslag_counts.most_common(3)
                        )
                        summary_bits.append(f"Top seasonal lags: {most_seas}")
                    if period_counts:
                        most_p = ", ".join(
                            f"P={k}({v})" for k, v in period_counts.most_common(3)
                        )
                ...
                fr = (
                    (ready / max(1, len(stationarity_df))) * 100
                    if not stationarity_df.empty
                    else 0
                )
                if fr > 70:
        ...
        if fe:
            ...
            if trans:
                ...
                def score_item(st):
                    s = 0
                    skew = abs(st.get("skewness", 0))
                    kurt = st.get("kurtosis", 0)
                    out = st.get("outliers_pct", 0)
                    cv = st.get("cv", 0)
                    if skew > 2:
                        s += 3
                    elif skew > 1:
            ...
            if enc:
                ...
                for f, info in enc.items():
                    ...
                    nullp = info.get("null_percentage", 0.0)
                    strat = (info.get("strategies") or ["OneHot"])[0]
                    if card <= 5:
                        simple.append((f, card, strat, nullp))
                    elif card <= 20:
                        complex_.append((f, card, strat, nullp))
                    else:
                        high.append((f, card, strat, nullp))
                if simple:
                    print("✅ Simple (≤5):")
                ...
                if high:
                    ...
                    for f, c, s, _ in high[:3]:
                        ...
                        if c > 100:
                            rec(
                                "Consider hashing or rare-category grouping",
                                "medium",
                                1,
                            )
        ...
        if miss:
            ...
            if hasattr(rates, "empty") and not rates.empty:
                ...
                if hi:
                    rec(f"{hi} features with >20% missing", "high")
                if mid:
                    rec(f"{mid} features with 5–20% missing", "medium")
        ...
        if dim:
            ...
            n_samples = int(chars.get("n_samples", 0) or 0)
            n_features = int(chars.get("n_features", 0) or 0)
            cond = chars.get("condition_number", np.nan)
            erank = chars.get("effective_rank", None)

            print("Analysis Overview:")
            metric("Embedding Methods", n_methods)
            metric("Samples × Features", f"{n_samples} × {n_features}")
            if np.isfinite(cond):
                metric("Condition Number", f"{cond:.2e}")
            if erank is not None:
                metric("Effective Rank", erank)

            # Preprocessing summary
            feats_removed = prep.get("features_removed")
            scaling = prep.get("scaling_method")
            final_shape = prep.get("final_shape")
            sampling = prep.get("sampling_method")

            section("Preprocessing", 3)
            if feats_removed is not None:
                metric("Low-Variance Removed", feats_removed)
            if scaling:
                metric("Scaling", scaling)
            if sampling:
                metric("Sampling", sampling)
            if final_shape:
                metric("Post-Preproc Shape", f"{final_shape[0]} × {final_shape[1]}")

            # ---------- Method Leaderboard ----------
            # Compute the same weighted score used in _generate_recommendations
            weights = {
            ...
            if scores:
                section("Method Leaderboard", 3)
                # Top 5 by score
                for name, sc in sorted(scores.items(), key=lambda kv: -kv[1])[:5]:
                    m = evals.get(name, {})
                    bits = []
                    if "silhouette_score" in m:
                        bits.append(f"sil={m['silhouette_score']:.3f}")
                    if "neighborhood_preservation" in m:
                        bits.append(f"ρ={m['neighborhood_preservation']:.3f}")
                    if "embedding_stability" in m:
                        bits.append(f"stab={m['embedding_stability']:.3f}")
                    metric(name, f"score={sc:.3f} | " + " ".join(bits), indent=1)

            # ---------- Per-Method Details ----------
            # Show concise, method-aware details (variance explained for PCA/SVD, params for nonlinear)
            section("Per-Method Details", 3)
            shown = 0
            for name, res in embeddings.items():
                info_bits = []
                mtype = res.get("method_type", "linear")
                n_components = res.get("n_components", None)
                if n_components:
                    info_bits.append(f"n_comp={n_components}")

                # Linear: explained variance
                evr = res.get("explained_variance_ratio", None)
                if isinstance(evr, (list, np.ndarray)) and len(evr) > 0:
                    tot = float(np.sum(evr))
                    info_bits.append(f"var={tot:.3f}")

                # Nonlinear: core params (neighbors/perplexity)
                if mtype == "nonlinear":
                    if "n_neighbors" in res and res["n_neighbors"] is not None:
                        info_bits.append(f"neighbors={res['n_neighbors']}")
                    if "perplexity" in res and res["perplexity"] is not None:
                        info_bits.append(f"perplexity={res['perplexity']}")

                # Evaluation quick glance
                m = evals.get(name, {})
                if "silhouette_score" in m:
                    info_bits.append(f"sil={m['silhouette_score']:.3f}")
                if "neighborhood_preservation" in m:
                    info_bits.append(f"ρ={m['neighborhood_preservation']:.3f}")
                if "embedding_stability" in m:
                    info_bits.append(f"stab={m['embedding_stability']:.3f}")

                metric(name, " | ".join(info_bits) if info_bits else "—", indent=1)
                shown += 1
                if shown >= 10:  # avoid long spam
                    remaining = len(embeddings) - shown
                    if remaining > 0:
                        print(f"   ... and {remaining} more")
                    break

            # ---------- PCA / Linear Summary (if available) ----------
            if "pca" in embeddings:
                p = embeddings["pca"]
                evr = p.get("explained_variance_ratio", None)
                if isinstance(evr, (list, np.ndarray)) and len(evr) > 0:
                    section("PCA Summary", 3)
                    cum = np.cumsum(evr)
                    k95 = (
                        int(np.argmax(cum >= 0.95) + 1)
                        if np.any(cum >= 0.95)
                        else len(evr)
                    )
                    metric("Variance ≥95%", f"{k95} components")
                    metric("Total Variance Explained", f"{float(np.sum(evr)):.3f}")

            # ---------- Visualization-Ready Embeddings ----------
            viz_ready = []
            for name, res in embeddings.items():
                emb = res.get("embedding")
                if emb is not None and hasattr(emb, "shape"):
                    if emb.shape[1] == 2:  # ready to plot in 2D
                        viz_ready.append(name)
            if viz_ready:
                section("2D Visualizations Available", 3)
                metric("Embeddings", ", ".join(viz_ready))

            # ---------- Recommendations ----------
            if recs:
                section("Recommendations", 3)
                for r in recs[:6]:
                    rec(str(r))
                if len(recs) > 6:
        ...
        if group_results and "error" not in group_results:
            ...
            if group_info:
                ...
                if group_info.get("missing_percentage", 0) > 10:
                    metric(
                        "Missing Data",
                        f"{group_info['missing_percentage']:.1f}%",
                        status="warning",
                        indent=1,
                    )
            ...
            if summary:
                ...
                if n_significant > 0:
                    significance_status = (
                        "excellent" if n_significant > total_vars * 0.3 else "good"
                    )
                    metric(
                        "Significant Variables",
                        f"{n_significant}/{total_vars}",
            ...
            if variable_analyses:
                ...
                for var_name, var_results in list(variable_analyses.items())[
                    :3
                    ...
                    if descriptives:
                        ...
                        for group_name, stats in descriptives.items():
                            ...
                            n_val = stats.get("n", 0)
                            print(
                                f"     • {group_name}: μ={mean_val:.3f} (±{std_val:.3f}), n={n_val}"
                            )
                    ...
                    if best_test:
                        ...
                        method_name = test_result.get(
                            "method", test_name.replace("_", " ").title()
                        )
                        p_value = test_result.get("p_value", 1.0)
                        significant = test_result.get("significant", False)
                    ...
                    if var_recommendations:
                        print("   Key Insights:")
                        for _rec in var_recommendations[
                            :2
            ...
            if categorical_associations:
                ...
                for cat_var, assoc_results in categorical_associations.items():
                    ...
                    if chi_square and "error" not in chi_square:
                        ...
                        metric(
                            "Cramér's V", f"{cramers_v:.3f} ({effect_size})", indent=1
                        )
                        ...
                        if fisher_exact and "error" not in fisher_exact:
                            ...
                            metric(
                                "Fisher's Exact p-value", f"{fisher_p:.6f}", indent=1
                            )
            ...
            if variable_analyses:
                ...
                for var_results in variable_analyses.values():
                    ...
                    levene_test = assumptions.get("homogeneity_tests", {}).get(
                        "levene", {}
                    )
                ...
                norm_pct = (
                    (total_variables - normality_violations) / total_variables * 100
                ...
                metric(
                    ...
                    f"{total_variables - normality_violations}/{total_variables} ({norm_pct:.0f}%)",
                    status=norm_status,
                )

                # Variance homogeneity assessment
                var_pct = (
                    (total_variables - variance_violations) / total_variables * 100
                    if total_variables > 0
                    else 0
                )
                var_status = (
                ...
                metric(
                    ...
                    f"{total_variables - variance_violations}/{total_variables} ({var_pct:.0f}%)",
                    status=var_status,
                )
                ...
                if normality_violations > total_variables * 0.5:
                    print(
                        "   💡 Many variables violate normality - non-parametric methods recommended"
                    )
                if variance_violations > total_variables * 0.5:
                    print(
                        "   🔧 Many variables have unequal variances - use Welch's tests"
                    )
            ...
            if summary.get("n_significant", 0) > 0:
                ...
                if n_sig == total_vars:
                    print(
                        f"   🌟 ALL variables show significant differences between {categorical_col}"
                    )
                elif n_sig > total_vars * 0.5:
                    print(
                        f"   ✅ MAJORITY of variables ({n_sig}/{total_vars}) show significant differences between {categorical_col}"
                    )
                else:
                    print(
                        f"   📊 SOME variables ({n_sig}/{total_vars}) show significant differences between {categorical_col}"
                    )

                print(
                    f"   📈 Consider using {categorical_col} as a strong predictor in your models"
                )
                ...
                categorical_col = group_info.get("categorical_column", "groups")
                print(
                    f"   ❌ NO significant differences found between {categorical_col}"
                )
                print(
                    f"   🤔 {categorical_col} may not be a useful predictor for these variables"
                )
        ...
        if graph_results and "error" not in graph_results:
            ...
            if data_info:
                ...
                metric(
                    "Original Data",
                    f"{original_shape[0]} samples × {original_shape[1]} features",
                )
                metric(
                    "Analyzed Data",
                    f"{analyzed_shape[0]} samples × {analyzed_shape[1]} features",
                )
            ...
            if summary:
                ...
                metric(
                    ...
                    f"{successful}/{total} ({success_rate:.0f}%)",
                ...
                if best_graph:
                    print(
                        f"   🏆 Best Network Type: {best_graph.replace('_', ' ').title()}"
                    )
                    ...
                    metric(
                        "Network Size",
                        f"{best_nodes} nodes, {best_edges} edges",
                        indent=1,
                    )
                    ...
                    metric(
                        "Connectivity",
                        "Fully Connected" if best_connected else "Disconnected",
            ...
            if topology_results and best_graph and best_graph in topology_results:
                ...
                if "error" not in topology:
                    ...
                    if degree_stats:
                        ...
                        metric(
                            "Degree Range",
                            f"{degree_stats.get('min', 0)} - {degree_stats.get('max', 0)}",
                            indent=1,
                        )
                        metric(
                            "Degree Std Dev",
                            f"{degree_stats.get('std', 0):.1f}",
                            indent=1,
                        )
                    ...
                    if assortativity is not None:
                        assort_status = (
                            "good" if abs(assortativity) < 0.3 else "warning"
                        )
                        metric(
                            "Degree Assortativity",
                            f"{assortativity:.3f}",
                            status=assort_status,
                            indent=1,
                        )
            ...
            if community_results and best_graph and best_graph in community_results:
                ...
                successful_methods = [
                    ...
                    if "error" not in communities[method]
                ]
                ...
                if successful_methods:
                    ...
                    if best_method:
                        ...
                        metric(
                            "Modularity Score",
                            f"{best_modularity:.3f}",
                            status=modularity_status,
                            indent=1,
                        )
                    ...
                    if len(successful_methods) > 1:
                        ...
                        for method in successful_methods:
                            ...
                            print(
                                f"     • {method.replace('_', ' ').title()}: {n_comm} communities (Q={modularity:.3f})"
                            )
                else:
            ...
            if centrality_results and best_graph and best_graph in centrality_results:
                ...
                for centrality_type in [
                    ...
                    if (
                        centrality_type in centralities
                        and "error" not in centralities[centrality_type]
                    ):
                ...
                if (
                    "pagerank" in centralities
                    and "error" not in centralities["pagerank"]
                ):
            ...
            if embedding_results and best_graph and best_graph in embedding_results:
                ...
                successful_embeddings = [
                    ...
                    if "error" not in embeddings[method]
                ]
                ...
                if successful_embeddings:
                    ...
                    if (
                        "node2vec" in embeddings
                        and "error" not in embeddings["node2vec"]
                    ):
                        similar_pairs = embeddings["node2vec"].get(
                            "most_similar_pairs", []
                        )
                        if similar_pairs:
                            print("\n🔗 Most Similar Variable Pairs (Node2Vec):")
                            for i, (var1, var2, similarity) in enumerate(
                                similar_pairs[:3], 1
                            ):
                                print(
                                    f"     {i}. {var1} ↔ {var2} (similarity: {similarity:.3f})"
                                )
                else:
            ...
            if len(graphs) > 1:
                ...
                for graph_type, graph_info in graphs.items():
                    if "error" not in graph_info:
                        ...
                        print(
                            f"   • {graph_type.replace('_', ' ').title()}: ❌ Failed ({error_msg})"
                        )
            ...
            if summary and best_graph:
                ...
                if best_edges > 0:
                    if best_connected and best_nodes >= 5:
                        print(
                            f"   🌟 STRONG network structure detected with {best_nodes} interconnected variables"
                        )
                        print(
                            f"   🔗 Rich connectivity ({best_edges} relationships) enables advanced graph-based analysis"
                        )
                    ...
                    if (
                        embedding_results.get(best_graph, {}).get("node2vec")
            ...
            print(
                "💡 Ensure you have at least 2 numeric columns with sufficient variance"
            )
        ...
        if self._categorical_cols:
            ...
            for col in self._categorical_cols[:8]:
                ...
                up = (u / (n or 1)) * 100
                nullp = self.df[col].isna().mean() * 100
                if up > 95:
                    st, desc = "warning", "Very high cardinality"
                    issues.append(f"{col}: {desc}")
                elif up > 50:
                    st, desc = "fair", "High cardinality"
                elif u < 2:
                    st, desc = "poor", "Constant/near-constant"
                    issues.append(f"{col}: {desc}")
                else:
                    st, desc = "good", "Appropriate cardinality"
                print(f"   • {col}:")
                metric("Unique Values", f"{u} ({up:.1f}%)", status=st, indent=1)
                if nullp > 0:
                    metric(
                        "Missing", f"{nullp:.1f}%", status=band(100 - nullp), indent=1
                    )
            if issues:
        ...
        label = (
            "Excellent - analysis-ready"
            if overall > 90
            else (
                "Good - minor preprocessing"
                if overall > 80
                else (
                    "Fair - moderate preprocessing"
                    if overall > 70
                    else "Poor - extensive preprocessing"
                )
            )
        )
        print(
        ...
        if completeness < 95:
            recs.append(
                (
                    (
                        "Handle missing data (impute/repair patterns)"
                        if completeness >= 80
                        else "Address critical missing data patterns"
                    ),
                    "high" if completeness < 80 else "medium",
                )
            )
        if uniqueness < 95:
            recs.append(("Investigate and handle duplicate records", "medium"))
        if not ds.empty:
            skewed = int(ds.get("is_skewed", False).sum())
            total = len(ds)
            if total and skewed > total * 0.5:
                recs.append(
                    ("Apply transformations to address widespread skewness", "high")
                )
            elif total and skewed > total * 0.3:
                recs.append(("Consider transformations for skewed features", "medium"))
        if out_sum:
            rate = out_sum.get("overall_outlier_rate", 0)
            if rate > 0.1:
                recs.append(
                    ("Implement robust outlier detection and treatment", "high")
                )
            elif rate > 0.05:
                recs.append(("Review and validate detected outliers", "medium"))
        cl_evals = (safe_get("clusters") or {}).get("evaluations", {})
        if cl_evals:
            best_sil = max(
                [e.get("silhouette_score", 0) for e in cl_evals.values()] + [0]
            )
            if best_sil > 0.7:
                recs.append(
                    ("Strong clustering structure — consider segmentation", "low")
                )
            elif best_sil > 0.5:
                recs.append(
                    ("Moderate clustering potential — explore segmentation", "low")
                )
        ...
        for text, p in recs:
            ...
            rec(text, p, 1)
        if not recs:
            rec("Dataset in excellent condition — proceed with modeling", "low")
