                              ...
                              <div className="rounded-2xl bg-black/20 p-3 text-xs text-slate-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                                {result.type === "metrics" ? (
                                  <div className="grid grid-cols-2 gap-2">
                                    {Object.entries((result as any).data || {}).slice(0, 4).map(([key, value]) => (
                                      <div key={key} className="rounded-xl bg-white/[0.04] p-2">
                                        <div className="truncate text-[10px] uppercase tracking-[0.14em] text-slate-500">{key}</div>
                                        <div className="mt-1 font-mono text-amber-300">{String(value)}</div>
                                      </div>
                                    ))}
                                  </div>
                                ) : result.type === "plot" ? (
